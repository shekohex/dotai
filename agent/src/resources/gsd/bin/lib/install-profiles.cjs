/**
 * Install profiles — single source of truth for which skills/agents are written to the runtime
 * config dirs.
 *
 * Background: every installed `gsd-*` skill costs eager system-prompt tokens because runtimes
 * (Claude Code, opencode, etc.) enumerate skill descriptions in `<available_skills>` on every turn.
 * With 86 skills + 33 agents the floor is ~12k tokens per turn, which is a meaningful tax for local
 * LLMs with 32K–128K context. Frontier models (Sonnet 4.6 / Opus 4.7 with 200K–1M ctx) don't feel
 * it.
 *
 * The `minimal` profile installs the main GSD loop only: new-project → discuss-phase → plan-phase →
 * execute-phase plus `help` (discoverability) and `update` (upgrade path).
 *
 * Users opt into minimal via `--minimal` on the install CLI. Default install (`full`) is unchanged
...
/**
 * Stage a filtered copy of the source commands/gsd directory when in minimal mode. All
 * runtime-specific copy fns recurse a source dir, so filtering at the source point lets every copy
 * fn stay unchanged (DRY: one filter, not 12).
 *
 * In full mode this is a no-op — the original srcDir is returned.
 *
 * Cleanup: the staged dir is automatically removed on process exit. If the copy loop throws
 * mid-flight, the partially-populated dir is removed and the error re-raised, so callers never see
 * an orphan.
 *
...
function stageSkillsForMode(srcDir, mode) {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      fs.copyFileSync(path.join(srcDir, entry.name), path.join(stageDir, entry.name));
    }
  } catch (err) {
    try {
