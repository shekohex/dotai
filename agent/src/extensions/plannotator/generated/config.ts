 ...
 * Plannotator Config
 *
 * Reads/writes ~/.plannotator/config.json for persistent user settings. Runtime-agnostic: uses only
...
export interface PlannotatorConfig {
  ...
  /**
   * Enable `gh attestation verify` during CLI installation/upgrade. Read by
   * scripts/install.sh|ps1|cmd on every run (not by any runtime code). When true, the installer
   * runs build-provenance verification after the SHA256 checksum check; requires `gh` CLI installed
   * and authenticated (`gh auth login`). OS-level opt-in only — no UI surface. Default: false.
   */
  verifyAttestation?: boolean;
  /**
   * Enable Jina Reader for URL-to-markdown conversion during annotation. When true (default),
   * `plannotator annotate <url>` routes through r.jina.ai for better JS-rendered page support and
...
/** Load config from ~/.plannotator/config.json. Returns {} on missing file or malformed JSON. */
...
/**
 * Save config by merging partial values into the existing file. Creates ~/.plannotator/ directory
 * if needed.
 */
...
/**
 * Detect the git user name from `git config user.name`. Returns null if git is unavailable, not in
 * a repo, or user.name is not set.
 */
...
/**
 * Build the serverConfig payload for API responses. Reads config.json fresh each call so the
 * response reflects the latest file on disk.
 */
 ...
 *
 * Priority (highest wins): --no-jina CLI flag → PLANNOTATOR_JINA env var → config.jina → default
 * true
 */
