 ...
 *
 * Runtime-agnostic: no node:fs, no node:http, no Bun APIs. Both the Bun server handler and (future)
 * Node handler import this module and wrap it with their respective HTTP transport layers.
 *
...
/**
 * Snapshot of the diff the reviewer was looking at when this job was launched. Carried on the job
 * so downstream UIs (agent-result panel "Copy All") export the same `**Diff:** ...` header the job
 * was actually run against — if the reviewer switches the UI to a different diff afterwards, the
 * job's snapshot still reflects truth. Structurally compatible with the UI-side
...
export interface AgentJobInfo {
  ...
  cwd?: string;
  /**
   * The review prompt text (system + user message). Stored separately from command for providers
   * that use stdin.
   */
  prompt?: string;
