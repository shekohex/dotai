...
Final voice transcripts use Pi `CustomEntry` persistence (`appendEntry` plus `registerEntryRenderer`), so `[live · you]` and `[live · Pi]` cards render in chat without entering model context or triggering turns. The `delegation.created` request is a separate visible `CustomMessageEntry` with a distinct `Pi Live → workspace` renderer and it intentionally triggers the Pi turn. The renderer marks whether the live model synthesized the task or copied the current transcript directly. Concise English tasks bypass normalization, while transcript-like English prose is synthesized into a standalone task. Non-English delegations are translated outside AgentSession through the fast fallback chain (`gpt-5.4-mini`, then `deepseek-v4-flash`, then remaining shared fallbacks) with minimal reasoning; the resulting English task triggers AgentSession while the original request appears underneath in muted UI-only text. OpenAI commentary-phase text is forwarded over the sideband commentary channel, final-answer text is kept separate, the main window is structurally titleless with no traffic lights or rectangular AppKit shadow, and intentional WebRTC closure is not reported as a failure. Its resizable style capability preserves key-window Space/Escape handling without a title bar.

Spoken turns lasting roughly one minute, containing at least 100 words, or exceeding 700 characters carry the complete transcript alongside the concise delegation task. English is attached verbatim. Non-English transcripts are translated completely in source-preserving chunks with up to 6,144 output tokens per chunk before delivery, so long Arabic turns retain their full context instead of only the short live-model delegation.
...
`live.jsonl` diagnostics are disabled by default. The native General → Diagnostics toggle is persisted locally, sent in pairing preferences, synchronized to `live.diagnosticsEnabled` in workspace `settings.json`, and can be changed during an active call. Turning it off stops future writes without deleting an existing log.

When `/live` runs inside an established session, every delegation uses `deliverAs: "steer"` with `triggerTurn: true`. An idle AgentSession starts a turn immediately; an active or settling AgentSession receives the delegation as steering input. The delegation becomes active only when its custom message emits `message_start`. Empty/error/aborted AgentSession completions are surfaced to the TUI and live model rather than silently treated as successful.

Pi 0.82 custom messages remain provider `user` messages so they drive a real task turn; the custom
renderer is presentation-only. Thinking-only/empty successful stops retry up to three times. English
prose copied directly from the transcript is normalized into a standalone task; concise commands
