...
/**
 * Per-tick projection of PreviewPane state. Replaces the prior `setNotesVisible(boolean)`
 * sliver-setter and the sibling reads of `optionListView.getSelectedIndex()` / `isFocused()`. The
 * pane now reads `selectedIndex` and `focused` from its own props — both derived from canonical
 * state via `selectPreviewPaneProps`. `OptionListView` and `PreviewPane` see the same source of
 * truth without the cross-component live read.
 */
...
/**
 * Thin layout composer. Receives `selectedIndex` / `focused` / `notesVisible` via `setProps` per
 * tick (computed by `selectPreviewPaneProps` from canonical state). Delegates option-side rendering
 * to `OptionListView` (which still owns its render-time state — input buffer, confirmedIndex) and
 * preview-side rendering to `PreviewBlockRenderer` (which owns the markdown cache and bordered-box
 * composition).
 *
 * `naturalHeight` and `maxNaturalHeight` query both children's heights; `render` combines them via
 * `decideLayout` (mode threaded into both calls — never re-derived).
 */
export class PreviewPane implements StatefulView<PreviewPaneProps>, Component {
  ...
  render(width: number): string[] {
    ...
    const stackedLines: string[] = [
      ...
      ...Array.from({ length: STACKED_GAP_ROWS }, () => ""),
