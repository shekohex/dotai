...
function isDifferentSession(
  ...
  if (origin.sessionId !== undefined && current.sessionId !== undefined) {
    return origin.sessionId !== current.sessionId;
  }
  if (origin.sessionFile !== undefined && current.sessionFile !== undefined) {
...
export function notifyCurrentPiSession(
  ...
  try {
    ...
    console.error(`Plannotator current-session notification failed: ${errorMessage(err)}`);
    return false;
  }
}
...
export function getCurrentPiSessionContext():
  | { pi: ExtensionAPI; ctx: ExtensionContext; identity?: PiSessionIdentity }
  | undefined {
  const current = getStore().current;
...
export function sendUserMessageToCurrentPiSession(
  ...
  | { ok: true }
  | { ok: false; reason: "no-current" | "same-session" | "send-failed"; error: unknown } {
  const current = getStore().current;
  if (!current) {
    return {
      ok: false,
