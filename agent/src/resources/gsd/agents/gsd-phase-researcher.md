...
If `brave_search: false` (or not set), use the available `websearch` capability instead.
...
@{{GSD_BUNDLE_DIR}}/references/thinking-models-research.md
...
```bash
node "{{GSD_BUNDLE_DIR}}/bin/gsd tools.cjs" graphify query "<capability-keyword>" --budget 1500
```
