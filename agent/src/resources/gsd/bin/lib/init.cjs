...
/**
 * Inject `project_root` into an init result object. Workflows use this to prefix `.planning/` paths
 * correctly when Claude's CWD differs from the project root (e.g., inside a sub-repo).
 */
...
function cmdInitNewMilestone(cwd, raw) {
  ...
  try {
    if (fs.existsSync(phasesDir)) {
      ...
      phaseDirCount = fs
        .readdirSync(phasesDir, { withFileTypes: true })
        .filter((entry) => entry.isDirectory() && isDirInMilestone(entry.name)).length;
    }
  } catch {}
 ...
 *
 * Returns the minimal set of fields that ingest-docs.md needs to detect whether a project/planning
 * dir exists and choose new vs merge mode. Mirrors the initIngestDocs SDK handler in
  ...
  for (const [num, name] of roadmapPhaseNames) {
    ...
    if (!seenPhaseNums.has(stripped)) {
      const checkboxComplete =
        roadmapCheckboxStates.get(num) === true || roadmapCheckboxStates.get(stripped) === true;
...
/**
 * Detect child git repos in a directory (one level deep). Returns array of { name, path,
 * has_uncommitted } objects.
 */
 ...
 *
 * Reads `config.agent_skills[agentType]` and validates each skill path exists within the project
 * root. Returns a formatted `<agent_skills>` block or empty string if no skills are configured.
 *
  ...
  for (const skillPath of skillPaths) {
    ...
    if (skillPath.startsWith("global:")) {
      ...
      if (!fs.existsSync(globalSkillMd)) {
        process.stderr.write(
          `[agent-skills] WARNING: Global skill not found at "~/.claude/skills/${skillName}/SKILL.md" — skipping\n`,
        );
        continue;
      }
      ...
      validPaths.push({
        ref: `${globalSkillDir}/SKILL.md`,
        display: `~/.claude/skills/${skillName}`,
      });
      continue;
    }
...
/**
 * Command: output the agent skills block for a given agent type. Used by workflows: SKILLS=$(node
 * "$TOOLS" agent-skills gsd-executor 2>/dev/null)
 */
 ...
 *
 * Scans the canonical skill discovery roots and returns a normalized inventory object with
 * discovered skills, root metadata, and installation summary flags. A legacy `skillsDir` override
 * is still accepted for focused scans, but the default mode is multi-root discovery.
 *
 ...
 *
 * Options: --skills-dir <path> Optional absolute path to a single skills directory --write Also
 * write to .planning/skill-manifest.json
 */
