 ...
 *
 * Renders profiling analysis into user-facing artifacts: - write-profile: USER-PROFILE.md from
 * analysis JSON - profile-questionnaire: fallback when no sessions available -
 * generate-dev-preferences: dev-preferences.md command artifact - generate-claude-profile:
 * Developer Profile section in CLAUDE.md - generate-claude-md: full CLAUDE.md with managed
 * sections
 */
...
function generateProjectSection(cwd) {
  ...
  if (!content) {
    return {
      content: CLAUDE_MD_FALLBACKS.project,
      source: "PROJECT.md",
      linkPath: null,
      hasFallback: true,
    };
...
function generateStackSection(cwd) {
  ...
  if (!content) {
    return {
      content: CLAUDE_MD_FALLBACKS.stack,
      source: "STACK.md",
      linkPath: null,
      hasFallback: true,
    };
...
function generateConventionsSection(cwd) {
  ...
  return {
    content: summary,
    source: "CONVENTIONS.md",
    linkPath: ".planning/codebase/CONVENTIONS.md",
    hasFallback: false,
  };
}
...
function generateArchitectureSection(cwd) {
  ...
  return {
    content: summary,
    source: "ARCHITECTURE.md",
    linkPath: ".planning/codebase/ARCHITECTURE.md",
    hasFallback: false,
  };
}
...
/**
 * Discover project skills from standard directories and extract frontmatter (name + description)
 * for each. Returns a table summary for CLAUDE.md so agents know which skills are available at
 * session startup (Layer 1 discovery).
 */
...
/**
 * Extract name and description from YAML-like frontmatter in a SKILL.md file. Handles multi-line
 * description values (continuation lines indented with spaces).
 */
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  if (options.global) {
    ...
    targetPath = path.isAbsolute(configClaudeMdPath)
      ? configClaudeMdPath
      : path.join(cwd, configClaudeMdPath);
  }
...
function cmdGenerateClaudeMd(cwd, options, raw) {
  ...
  let outputPath = options.output;
  if (!outputPath) {
    outputPath = path.isAbsolute(configClaudeMdPath)
      ? configClaudeMdPath
      : path.join(cwd, configClaudeMdPath);
  } else if (!path.isAbsolute(outputPath)) {
    outputPath = path.join(cwd, outputPath);
    ...
    for (const name of MANAGED_SECTIONS) {
      ...
      if (hasMarkers) {
        if (options.auto) {
          const effectiveMode = blockModes[name] || globalAssemblyMode;
          const expectedBody =
            effectiveMode === "link" && gen.linkPath && !gen.hasFallback
              ? `${heading}\n\n@${gen.linkPath}`
              : `${heading}\n\n${gen.content}`;
