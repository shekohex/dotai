...
/**
 * Per-tick projection of SubmitPicker state. The picker is a fixed 2-row structure (Submit /
 * Cancel) — labels are static, only the active marker varies per tick. `selectSubmitPickerProps`
 * precomputes `active` per row.
 */
 ...
 * - Both rows render in normal style at all times — D1 (revised) allows partial submission, so Submit
 *   is never dimmed or visually marked as unselectable. The warning header in
 *   `buildSubmitContainer` is the sole signal of incompleteness.
 * - `naturalHeight(width)` is state-INDEPENDENT and returns a constant 2, so the chrome-mirror layout
 *   in `buildSubmitContainer` can subtract a fixed 2 lines without re-rendering.
 */
export class SubmitPicker implements StatefulView<SubmitPickerProps> {
  ...
  render(width: number): string[] {
    ...
    for (let i = 0; i < 2; i++) {
      const text =
        i === 0 ? uiText("submit.label", SUBMIT_LABEL) : uiText("submit.cancel", CANCEL_LABEL);
