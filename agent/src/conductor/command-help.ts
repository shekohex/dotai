...
export function helpText(topic?: string): string {
  ...
  return [
    ...
    "  projectScanIntervalSeconds: pollingIntervalSeconds",
    "  webhookPollingIntervalSeconds: 900",
    "  webhookProjectScanIntervalSeconds: 1800",
    "  repositories[].webhookEnabled: false",
    ...
    "  GitHub UI: application/json, Enable SSL verification, Let me select individual events.",
    "  Repo webhook UI: select Issues, Issue comments, Pull requests, PR reviews, PR review comments, Check runs, Check suites, Statuses, Workflow runs, Pushes.",
    "  projects_v2_item is organization-webhook only; repository webhooks cannot subscribe.",
    "  Project polling remains required for repository webhooks and user-owned projects.",
    "  Set repositories[].webhookEnabled=true only after that repository webhook is active.",
    "  Repositories without webhookEnabled keep normal polling and project scans.",
    "  Pushes refresh mergeability only when the pushed branch is an active run's PR base branch.",
    "",
    "Daemon files:",
