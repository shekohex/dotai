...
const isActiveTab: PerTabSelector<boolean> = (s, ctx) =>
  ctx.i === Math.min(s.currentTab, Math.max(0, ctx.totalQuestions - 1));

/**
 * Pure factory: assembles every TUI component, the props adapter, and a lifecycle handle.
 * Session-state dependencies arrive via `getCurrentTab` and the `inputBuffer` cell. Initial paint
 * is delegated to `adapter.apply(initialState)` (called by the session at construction-end); no
 * selector is invoked here.
 *
 * @param {QuestionnaireBuildConfig} config - Build configuration.
 * @returns {QuestionnaireBuilt} Built questionnaire components.
 */
...
/**
 * Programming-by-intention assembly: each private method names one construction step from
 * `build()`. Read top-down; the class is build-only and discarded once the handle is returned.
 */
class QuestionnaireBuilder {
  ...
  private buildTabFor(question: QuestionData, index: number): TabComponents {
    ...
    const multiSelect =
      question.multiSelect === true ? new MultiSelectView(this.theme, question) : undefined;
    const bodyHeights = this.buildBodyHeights(question, preview, multiSelect);
    return { optionList, preview, multiSelect, bodyHeights };
  }

  /**
   * Build body height function for a question tab.
   *
   * @param {QuestionData} question - Question data.
   * @param {PreviewPane} preview - Preview pane.
   * @param {MultiSelectView | undefined} multiSelect - Multi-select view, if applicable.
   * @returns {(width: number) => TabBodyHeights} Function computing body heights for a given width.
   */
  ...
  /**
   * Compute cross-tab max adaptive left width and inject into each PreviewPane. Mirrors
   * buildHeightComputers pattern — iterates all tabs, takes max. Called after buildTabComponents,
   * before buildDialog (so setGlobalLeftWidth is set before any rendering occurs).
   *
   * @param {ReadonlyArray<TabComponents>} tabs - All tab components.
   */
  private injectGlobalLeftWidth(tabs: ReadonlyArray<TabComponents>): void {
    const questions = this.questions;
    const itemsByTab = this.itemsByTab;
    const tabsDescriptor = questions.map((q) => ({
  ...
  private pickInitialActivePreview(tabs: ReadonlyArray<TabComponents>) {
    const idx = Math.min(this.initialState.currentTab, Math.max(0, this.questions.length - 1));
  ...
  private buildDialog(
    ...
    return new DialogView(
      ...
      },
      {
        ...
        activePreviewPane: this.pickInitialActivePreview(tabs),
      },
    );
  }

  /**
   * Build global bindings for common components.
   *
   * @param {DialogView} dialog - Dialog view.
   * @param {SubmitPicker | undefined} submitPicker - Submit picker, if multi-question.
   * @param {TabBar | undefined} tabBar - Tab bar, if multi-question.
   * @returns {ReadonlyArray<BoundGlobalBinding>} Global binding descriptors.
   */
      ...
      ...(submitPicker
        ? [
            globalBinding({
              ...
              select: selectSubmitPickerProps,
            }),
          ]
        : []),
