...
/**
 * Scan immediate child directories for separate git repos. Returns a sorted array of directory
 * names that have their own `.git`. Excludes hidden directories and node_modules.
 */
 ...
 *
 * In multi-repo workspaces, Claude may open inside a sub-repo (e.g. `backend/`) instead of the
 * project root. This function prevents `.planning/` from being created inside the sub-repo by
 * locating the nearest ancestor that already has a `.planning/` directory.
 *
 ...
 *
 * Returns `startDir` unchanged when no ancestor `.planning/` is found (first-run or single-repo
 * projects).
 */
...
/**
 * Remove stale gsd-* temp files/dirs older than maxAgeMs (default: 5 minutes). Runs
 * opportunistically before each new temp file write to prevent unbounded accumulation.
 *
 ...
 */
/**
 * Dedicated GSD temp directory: path.join(os.tmpdir(), 'gsd'). Created on first use. Keeps GSD temp
 * files isolated from the system temp directory so reap scans only GSD files (#1975).
 */
...
/**
 * Frozen enum of typed reason codes used by error() for structured errors. Each subcommand
 * contributes its own codes; the enum exists so tests can assert against typed values instead of
 * grepping stderr (#2974).
 *
 * Adding a new code:
 *
 * - Pick a snake_case lowercase value (the JSON wire form)
 * - Group by subsystem prefix (CONFIG__, SDK__, etc)
...
/**
 * Process-level flag: when true, error() emits structured JSON to stderr instead of plain "Error:
 * <message>" text. Set by gsd-tools.cjs when the CLI is invoked with `--json-errors`. Tests opt in
 * to typed-IR error assertions by passing that flag and parsing the JSON.
 *
 * Default off so existing callers and human operators keep their plain-text diagnostics. The
 * structured form is opt-in for tooling and tests (#2974).
 */
...
function getJsonErrorMode() {
  return _jsonErrorMode;
}

/**
 * Emit an error and exit. When the second argument is provided it must be a value from
 * ERROR_REASON; tests can assert on `result.reason`. When the process is in JSON-error mode, stderr
 * receives `{ ok: false, reason, message }` so callers can parse it; otherwise stderr keeps the
 * plain text form for human operators.
 */
...
/**
 * Deep-merge two plain config objects. `overlay` wins on key conflict. Explicit `null` in overlay
 * overrides base (null means "unset this key"). Arrays are replaced, not merged. Non-object
 * primitives use overlay value.
 *
 * Note: `undefined` in overlay is treated as "no value provided" and falls back to base (preserves
 * inheritance). Explicit `null` overrides base.
 */
...
function loadConfig(cwd) {
  ...
  try {
    ...
    if ("depth" in fileData && !("granularity" in fileData)) {
      ...
      try {
        fs.writeFileSync(configPath, JSON.stringify(fileData, null, 2), "utf-8");
      } catch {
        /* intentionally empty */
      }
    }
    ...
    return {
      ...
      post_planning_gaps:
        get("post_planning_gaps", { section: "workflow", field: "post_planning_gaps" }) ??
      ...
      _auto_chain_active:
        get("_auto_chain_active", { section: "workflow", field: "_auto_chain_active" }) ?? false,
    ...
    try {
      ...
      return {
        ...
        post_planning_gaps:
          globalDefaults.post_planning_gaps ??
...
/**
 * Normalize markdown to fix common markdownlint violations. Applied at write points so
 * GSD-generated .planning/ files are IDE-friendly.
 *
 * Rules enforced: MD022 — Blank lines around headings MD031 — Blank lines around fenced code blocks
 * MD032 — Blank lines around lists MD012 — No multiple consecutive blank lines (collapsed to 2 max)
...
/**
 * Resolve the main worktree root when running inside a git worktree. In a linked worktree,
 * .planning/ lives in the main worktree, not in the linked one. Returns the main worktree path, or
 * cwd if not in a worktree.
 */
...
/**
 * Parse `git worktree list --porcelain` output into an array of { path, branch } objects. Entries
 * with a detached HEAD (no branch line) are skipped because we cannot safely reason about their
 * merge status.
 *
...
/**
 * Remove linked git worktrees whose branch has already been merged into the current HEAD of the
 * main worktree. Also runs `git worktree prune` to clear any stale references left by
 * manually-deleted worktree directories.
 *
 * Safe guards: - Never removes the main worktree (first entry in --porcelain output). - Never
 * removes the worktree at process.cwd(). - Never removes a worktree whose branch has unmerged
 * commits. - Skips detached-HEAD worktrees (no branch name).
 *
 * @param {string} repoRoot - Absolute path to the main (or any) worktree of the repository; used as
...
function pruneOrphanedWorktrees(repoRoot) {
  ...
  try {
    ...
    for (let i = 1; i < worktrees.length; i++) {
      ...
      const ancestorCheck = execGit(repoRoot, ["merge-base", "--is-ancestor", branch, "HEAD"]);
...
/**
 * Extract the phase token from a directory name. Supports: '01-name', '1009A-name', '999.6-name',
 * 'CK-01-name', 'PROJ-42-name'. Returns the token portion (e.g. '01', '1009A', '999.6', 'PROJ-42')
 * or the full name if no separator.
 */
...
/**
 * Check if a directory name's phase token matches the normalized phase exactly. Case-insensitive
 * comparison for the token portion.
 */
...
/**
 * Strip shipped milestone content wrapped in <details> blocks. Used to isolate current milestone
 * phases when searching ROADMAP.md for phase headings or checkboxes — prevents matching archived
 * milestone phases that share the same numbers as current milestone phases.
 */
 ...
 *
 * Instead of stripping <details> blocks (negative heuristic that breaks if agents wrap the current
 * milestone in <details>), this finds the section matching the current milestone version and
 * returns only that content.
 *
...
/**
 * Replace a pattern only in the current milestone section of ROADMAP.md (everything after the last
 * </details> close tag). Used for write operations that must not accidentally modify archived
 * milestone checkboxes/tables.
 */
...
/**
 * Check which GSD agents are installed on disk. Returns an object with installation status and
 * details.
 *
...
/**
 * Map short model aliases to full model IDs. Updated each release to match current model versions.
...
/**
 * #2517 — runtime-aware tier resolution. Maps `model_profile` tiers (opus/sonnet/haiku) to
 * runtime-native model IDs and (where supported) reasoning_effort settings.
 *
 * Each entry: { model: <id>, reasoning_effort?: <level> }
 *
 * `claude` mirrors MODEL_ALIAS_MAP — present for symmetry so `runtime: "claude"` resolves through
 * the same code path. `codex` defaults are taken from the spec in #2517. Unknown runtimes fall back
 * to the Claude alias to avoid emitting provider-specific IDs the runtime cannot accept.
 */
...
/**
 * Tier enum allowed under `model_profile_overrides[runtime][tier]`. Mirrors the regex in
 * `config-schema.cjs` (DYNAMIC_KEY_PATTERNS) so loadConfig surfaces the same constraint at read
 * time, not only at config-set time (review finding #10).
 */
const RUNTIME_OVERRIDE_TIERS = new Set(["opus", "sonnet", "haiku"]);

/**
 * Allowlist of runtime names the install pipeline currently knows how to emit native model IDs for.
 * Synced with `getDirName` in `bin/install.js` and the runtime list in `docs/CONFIGURATION.md`.
 * Free-string runtimes outside this set are still accepted (#2517 deliberately leaves the runtime
 * field open) — a warning fires once at loadConfig so a typo like `runtime: "codx"` does not
...
/**
 * Emit a one-time stderr warning for unknown runtime/tier keys in a parsed config blob. Idempotent
 * across calls — the same (file, key) pair only warns once per process so loadConfig can be called
 * repeatedly without spamming.
 *
 * Does NOT reject — preserves back-compat for users on a runtime not yet in the allowlist (the
 * new-runtime case must always be possible without code changes).
 */
  ...
  if (runtime && typeof runtime === "string" && !KNOWN_RUNTIMES.has(runtime)) {
    ...
    if (!_warnedConfigKeys.has(key)) {
      ...
      try {
        ...
        /* stderr might be closed in some test harnesses */
      }
    }
  }
 ...
 *
 * Single source of truth shared by core.cjs (resolveModelInternal / resolveReasoningEffortInternal)
 * and bin/install.js (Codex/OpenCode TOML emit paths). Always merges built-in defaults with user
 * overrides at the field level so partial overrides keep the unspecified fields:
 *
 * `{ codex: { opus: "gpt-5-pro" } }` keeps reasoning_effort: 'xhigh' `{ codex: { opus: {
 * reasoning_effort: 'low' } } }` keeps model: 'gpt-5.4'
 *
 * Without this field-merge, the documented string-shorthand example silently dropped
 * reasoning_effort and a partial-object override silently dropped the model — both reported as
 * critical findings in the #2609 review.
 *
 * Inputs:
 *
...
/**
 * Convenience wrapper used by resolveModelInternal / resolveReasoningEffortInternal. Pulls runtime
 * + overrides out of a loaded config and delegates to resolveTierEntry.
 */
...
function resolveModelInternal(cwd, agentType) {
  ...
  const phaseTypeTier =
    phaseType && config.models && typeof config.models === "object"
  ...
  // tier win.
  const tier =
    phaseTypeTier && VALID_TIERS.has(phaseTypeTier)
      ? phaseTypeTier
      : profile === "inherit"
        ? "inherit"
        : agentModels
 ...
 *
 * The orchestrator (workflow agent) tracks the attempt counter. On the first spawn, it calls with
 * attempt=0. If the orchestrator detects a soft failure (verification inconclusive, plan-check
 * FLAG, etc.), it re-spawns with attempt=1, which escalates the agent's tier one step up.
 * `max_escalations` caps how many escalations are allowed.
 *
 * Resolution precedence (highest → lowest):
 *
 * 1. Config.model_overrides[agent] (full IDs accepted)
 * 2. Dynamic_routing.tier_models[escalated_tier] (when enabled)
 * 3. Models[phase_type] / model_profile (existing chain via resolveModelInternal)
 *
 * When dynamic_routing is null/disabled, this function is identical to resolveModelInternal —
 * orchestrators can call it unconditionally without breaking back-compat.
 *
 * @param {string} cwd - Project directory.
 * @param {string} agentType - Agent name (e.g. 'gsd-verifier').
 * @param {number} [attempt=0] - 0 for first spawn; 1+ for escalation. Capped internally at
 *   max_escalations. Default is `0`
...
function resolveModelForTier(cwd, agentType, attempt) {
  ...
  const maxEscalations =
    Number.isInteger(dr.max_escalations) && dr.max_escalations >= 0 ? dr.max_escalations : 1;
  const escalationEnabled = dr.escalate_on_failure !== false;
  const effectiveAttempt = escalationEnabled ? Math.min(attemptN, maxEscalations) : 0;
...
/**
 * #2517 — Resolve runtime-specific reasoning_effort for an agent. Returns null unless:
 *
...
/**
 * Extract a one-liner from the summary body when it's not in frontmatter. The summary template
 * defines one-liner as a bold markdown line after the heading:
 *
...
/**
 * Returns a filter function that checks whether a phase directory belongs to the current milestone
 * based on ROADMAP.md phase headings. If no ROADMAP exists or no phases are listed, returns a
 * pass-all filter.
 */
...
/**
 * Read a phase directory and return counts/flags for common file types. Returns an object with
 * plans[], summaries[], and boolean flags for research/context/verification files.
 */
...
/**
 * Read immediate child directories from a path. Returns [] if the path doesn't exist or can't be
 * read. Pass sort=true to apply comparePhaseNum ordering.
 */
 ...
 *
 * On POSIX systems, `fs.renameSync` is atomic when the source and destination are on the same
 * filesystem. This prevents a process killed mid-write from leaving a truncated file that is
 * unparseable on next read.
 *
 * The temp file is placed alongside the target so it is guaranteed to be on the same filesystem
 * (required for rename atomicity). The PID is embedded in the temp file name so concurrent writers
 * use distinct paths.
 *
 * If `renameSync` fails (e.g. cross-device move), the function falls back to a direct
 * `writeFileSync` so callers always get a best-effort write.
 *
 * @param {string} filePath Absolute path to write.
 * @param {string | Buffer} content File content.
 * @param {string} [encoding='utf-8'] Encoding passed to writeFileSync. Default is `'utf-8'`
 */
