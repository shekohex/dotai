...
Mark a shipped version (v1.0, v1.1, v2.0) as complete. Creates historical record in local milestone archives under `.planning/milestones/`, performs full PROJECT.md evolution review, reorganizes ROADMAP.md with milestone groupings, and tags the release in git.
...
3. Record in the local milestone archive summary: `Known deferred items at close: {count} (see STATE.md Deferred Items)`
...
```bash
gsd-sdk query commit "chore: archive v[X.Y] milestone files" --files .planning/milestones/v[X.Y]-ROADMAP.md .planning/milestones/v[X.Y]-REQUIREMENTS.md .planning/milestones/v[X.Y]-MILESTONE-AUDIT.md .planning/PROJECT.md .planning/STATE.md .planning/ROADMAP.md
```
...
**If doesn't exist:** Create from template at `{{GSD_BUNDLE_DIR}}/templates/retrospective.md`.
...
- [ ] Known deferred items count noted in local milestone archive summary

- [ ] Local milestone archive summary created with stats and accomplishments
...
- [ ] Known gaps recorded in local milestone archive summary if user proceeded with incomplete requirements
