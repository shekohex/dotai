...
function parsePayload(payload: unknown): UnknownRecord | null {
  ...
  if (typeof payload === "string") {
    try {
      parsed = parseUnknownJson(payload);
    } catch {
      return null;
    }
  }
...
function parseSessionEvent(
  ...
  if (!isUnknownRecord(session) || typeof session.id !== "string") return null;
...
function parseTranscriptAddedEvent(
  ...
  if (!isUnknownRecord(item) || typeof item.text !== "string") return null;
  return { type, item: { text: item.text } };
}

function parseTurnDoneEvent(payload: UnknownRecord): LiveServerEvent | null {
  const turn = payload.turn;
  if (!isUnknownRecord(turn) || (turn.role !== "user" && turn.role !== "assistant")) return null;
...
function parseErrorEvent(payload: UnknownRecord): LiveServerEvent | null {
  ...
  if (isUnknownRecord(error) && typeof error.message === "string") {
 ...
 *
 * @param {unknown} payload Raw wire payload.
 * @returns {LiveServerEvent | null} Parsed event, or null for malformed payloads.
 */
 ...
 *
 * @param {string} instructions Live model instructions.
 * @param {string} voice OpenAI voice identifier.
 * @returns {LiveSessionPayload} Signaling session payload.
 */
...
/**
 * Build context associated with a server-created delegation.
 *
 * @param {string} delegationItemId Server delegation identifier.
 * @param {string} text Context text.
 * @param {LiveContextChannel | undefined} channel Optional semantic channel.
 * @returns {LiveClientMessage} Sideband context message.
 */
...
/**
 * Build context appended outside a delegation.
 *
 * @param {string} text Context text.
 * @param {LiveContextChannel | undefined} channel Optional semantic channel.
 * @returns {LiveClientMessage} Sideband context message.
 ...
 *
 * @param {string} text Context text.
 * @returns {string[]} UTF-8-safe chunks.
 */
