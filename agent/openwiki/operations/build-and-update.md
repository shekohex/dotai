...
| Patch                                                                 | Target                                                                | Effect                                                                                                                                                                                                                                                                                    |
| --------------------------------------------------------------------- | --------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `@earendil-works+pi-ai+0.84.2.patch`                                  | `pi-ai` retry utils + Fast Mode request handling                      | Adds retryable error patterns: `request failed`, `failed after retries`, `no response body`, `websocket transport is not available`, `stream closed before response.completed`, `invalid codex sse json`, `invalid codex websocket json`, `error occurred while processing your request`. Fast Mode hunks make stock providers honor `/fast`: preserve extension-provided `originator` headers, carry the requested `service_tier` through `streamSimple` and response processing, and price a response reporting `default` as the requested tier. Drop the Fast Mode hunks once upstream preserves these natively; retry hunks are independent and must survive regeneration. |
| `@earendil-works+pi-coding-agent++@earendil-works+pi-ai+0.84.2.patch` | the **nested** copy of `pi-ai` inside `pi-coding-agent/node_modules/` | Identical retry-pattern + Fast Mode additions (handles npm's non-deduped nested install). |
| `@earendil-works+pi-coding-agent+0.84.2.patch`                        | `pi-coding-agent/dist/modes/interactive/components/tool-execution.js` | Removes the internal leading spacer from tool execution rendering.                                                                                                                                                                                                                        |
