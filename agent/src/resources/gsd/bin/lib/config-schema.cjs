 ...
 *
 * Imported by: - config.cjs (isValidConfigKey validator) - tests/config-schema-docs-parity.test.cjs
 * (CI drift guard)
 *
 * Adding a key here without documenting it in docs/CONFIGURATION.md will fail the parity test.
 * Adding a key to docs/CONFIGURATION.md without adding it here will cause config-set to reject it
 * at runtime.
 */
...
/**
 * Dynamic-pattern validators — keys matching these regexes are also accepted. Each entry has a
 * `test` function and a human-readable `description`.
 */
const DYNAMIC_KEY_PATTERNS = [
  ...
  // plus a tier_models sub-block keyed by light/standard/heavy.
  {
    ...
    test: (k) =>
      /^dynamic_routing\.(enabled|escalate_on_failure|max_escalations|tier_models\.(light|standard|heavy))$/.test(
        k,
      ),
    description:
