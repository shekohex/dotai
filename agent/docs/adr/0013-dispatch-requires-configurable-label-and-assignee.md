...
If reconciliation sees a closed project item with an existing active run, Conductor first checks the run's branch for a merged PR. Merged PR finalization wins and marks the run `done`; otherwise Conductor marks the stale run blocked and stops its Herdr pane best-effort before normal active-run recovery can relaunch it.
