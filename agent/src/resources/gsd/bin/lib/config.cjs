...
function validateKnownConfigKeyPath(keyPath) {
  ...
  if (suggested) {
    error(
      `Unknown config key: ${keyPath}. Did you mean ${suggested}?`,
 ...
 *
 * Uses the canonical `git` namespace for branching keys (consistent with VALID_CONFIG_KEYS and the
 * settings workflow). loadConfig() handles both flat and nested formats, so this is
 * backward-compatible with existing projects that have flat keys.
 *
 ...
 *
 * Accepts user-chosen settings as a JSON string (the keys the user explicitly configured during
 * /gsd-new-project). All remaining keys are filled from hardcoded defaults and optional
 * ~/.gsd/defaults.json.
 *
...
function cmdConfigSet(cwd, keyPath, value, raw) {
  ...
  if (keyPath === "workflow.drift_action" && !VALID_DRIFT_ACTIONS.includes(String(parsedValue))) {
    error(
      `Invalid workflow.drift_action '${value}'. Valid values: ${VALID_DRIFT_ACTIONS.join(", ")}`,
    );
  }
  ...
  if (isSecretKey(keyPath)) {
    ...
    const maskedPrev =
      setConfigValueResult.previousValue === undefined
        ? undefined
...
/**
 * Schema-level defaults for well-known config keys. When a key is absent from config.json and no
 * --default flag was supplied, cmdConfigGet checks here before emitting "Key not found".
 */
...
/**
 * Print the resolved config.json path (workstream-aware). Used by settings.md so the workflow
 * writes/reads the correct file when a workstream is active (#2282).
 */
