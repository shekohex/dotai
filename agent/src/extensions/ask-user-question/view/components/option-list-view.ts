...
/**
 * Per-tick projection of OptionListView state. After Phase 11b, `inputBuffer` is part of the props
 * bag — the session-owned `inlineInput` (a headless `pi-tui` Input instance) supplies its current
 * `getValue()` here per tick. `OptionListView` is purely props-driven; the imperative buffer
 * surface and read-back getters are gone.
 */
...
/**
 * Sole owner of the option list's interactive state. Wraps a single `WrappingSelect`. Implements
 * `StatefulView<OptionListViewProps>`: `setProps` is the only mutator; render output reflects the
 * last props received.
 */
