 ...
 *
 * The /gsd-update workflow's check_latest_version step was previously prescribed in LLM-driven
 * prose ("run `npm view get-shit-done-cc version`"). The executing model could shortcut the
 * prescription and invent npm queries against wrong-shaped names (`@get-shit-done/cli`,
 * `get-shit-done-cli`, `gsd`), all of which 404 or — worse — return an unrelated typosquat
 * package.
 *
 * This script makes the package name a CONSTANT in code, not a free choice at execution time. The
 * workflow calls it via `npm run check-latest-version -- --json` and parses the structured
 * response.
 *
 * Tests assert on the typed CHECK_REASON enum and the structured result record, never on console
 * prose. See CONTRIBUTING.md "Prohibited: Raw Text Matching on Test Outputs".
 */
...
/**
 * Pure-ish: takes an injected spawn function so tests don't actually run npm. In production,
 * defaults to cp.spawnSync('npm', ...).
 */
function checkLatestVersion(opts = {}) {
  const defaultSpawn = () =>
    cp.spawnSync("npm", ["view", PACKAGE_NAME, "version"], {
      encoding: "utf8",
