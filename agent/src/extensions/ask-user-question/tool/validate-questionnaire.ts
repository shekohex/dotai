...
/**
 * Pure runtime validator for `QuestionParams`. Covers every guard except `no_ui` (which depends on
 * `ctx.hasUI` and stays inline at the call site). `reserved_label` MUST short-circuit before
 * `duplicate_option_label`.
 *
 * @param {QuestionParams} typed - The question parameters to validate
 * @returns {ValidationResult} Validation result with ok/error/message
 */
