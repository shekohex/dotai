...
import { QuestionTabStrategy, type TabContentStrategy } from "./tab-content-strategy.js";
import { SubmitTabStrategy } from "./components/submit-tab-strategy.js";
...
/**
 * `HINT_SINGLE` / `HINT_MULTI` describe the CORE hint that must always be visible — the collapse
 * affordance is appended AFTER cancel by `buildHintText` so the core stays a contiguous prefix
 * substring of the rendered line. On narrow terminals the collapse tail clips with `…`
 * (`OneLineClippedText`); the core is preserved.
 */
...
/**
 * Single-line footer shown by `QuestionnaireSession` when `state.collapsed === true`. Bypasses
 * `buildHintText`.
 */
...
export interface DialogConfig {
  ...
  /**
   * Optional so single-question mode and non-submit tests can omit it; SubmitTabStrategy falls back
   * to Spacer rows.
   */
  ...
  /**
   * Body height of the CURRENTLY active tab/option. The chrome subtracts this from `getBodyHeight`
   * to absorb the residual OUTSIDE the bordered region.
   */
...
/**
 * The 7th renderable, promoted from a structural literal to a named class so all view-layer
 * components share one explicit `implements StatefulView<P>` contract. `setProps(DialogProps)`
 * writes the live cell read by the strategy thunk during `render()`. `liveProps.activePreviewPane`
 * is a resolved pane reference threaded by the adapter per tick — the dialog itself does not derive
 * it.
 */
export class DialogView implements StatefulView<DialogProps> {
  ...
  render(width: number): string[] {
    ...
    if (natural.length + spacerRows <= termRows) {
      ...
      return spacerRows > 0
        ? [...natural, ...Array.from({ length: spacerRows }, () => "")]
        : natural;
    }
