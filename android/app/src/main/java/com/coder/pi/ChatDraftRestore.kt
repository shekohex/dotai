...
data class PendingChatSubmitStash(val text: String, val baselineSeq: Long?)
...
fun TerminalAgentStateSnapshot.latestAgentSeq(): Long? = listOfNotNull(
    hello?.seq,
    session?.event?.seq,
    run?.event?.seq,
    turn?.event?.seq,
...
fun observedAgentEventAcceptsPendingSubmit(pending: PendingChatSubmitStash, eventSeq: Long?): Boolean {
