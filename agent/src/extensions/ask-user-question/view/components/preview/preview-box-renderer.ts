...
const ANSI_OSC8_RE = new RegExp(`${ESC}\\]8;[^${BEL}${ESC}]*(?:${BEL}|${ESC}\\\\)`, "g");
...
/**
 * Drops fenced-code-block markers from rendered markdown.
 *
 * @param {readonly string[]} lines - Lines to filter
 * @returns {string[]} Lines without fence markers
 */
...
/**
 * Wraps `lines` in a 4-sided ASCII border with 1 col of inner horizontal padding. Layout per
 * content row: `│` + ` ` + content padded to `contentInner` + ` ` + `│`, where `contentInner =
 * width - BORDER_HORIZONTAL_OVERHEAD - 2 * BORDER_INNER_PADDING_HORIZONTAL`. Top/bottom dash runs
 * span corner-to-corner (`width - BORDER_HORIZONTAL_OVERHEAD`). When `hidden > 0`, the bottom-row
 * dash run is replaced with `✂ ── N lines hidden ──` (corners stay).
 *
 * @param {readonly string[]} lines - Content lines to box
 * @param {number} width - Total box width
 * @param {(s: string) => string} colorFn - Color function for border characters
 * @param {number} hidden - Number of hidden lines indicator
 * @returns {string[]} Rendered box lines
 */
 ...
 *
 * CC parity: contentWidth = max(minWidth, widestRenderedLine) boxWidth = min(contentWidth + 4,
 * effectiveMaxWidth)
 *
 * Trailing whitespace is stripped before measuring because pi-tui's `Markdown.render(width)` pads
 * every line to `width`, which would otherwise force the box to fill the whole column allocation.
 *
 * @param {readonly string[]} contentLines - Rendered content lines
 * @param {number} maxInnerWidth - Maximum inner width
 * @returns {{ innerWidth: number; boxWidth: number }} InnerWidth and boxWidth
 */
