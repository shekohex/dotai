...
/**
 * Replace a STATE.md field with fallback field name support. Tries `primary` first, then `fallback`
 * (if provided), returns content unchanged if neither matches. This consolidates the
 * replaceWithFallback pattern that was previously duplicated inline across phase.cjs,
 * milestone.cjs, and state.cjs.
 */
...
/**
 * Update fields within the ## Current Position section of STATE.md. This keeps the Current Position
 * body in sync with the bold frontmatter fields. Only updates fields that already exist in the
 * section; does not add new lines. Fixes #1365: advance-plan could not update Status/Last activity
 * after begin-phase.
 */
...
function cmdStateUpdateProgress(cwd, raw) {
  ...
  if (updated) {
    output(
      { updated: true, percent, completed: _totalSummaries, total: _totalPlans, bar: progressStr },
      raw,
      progressStr,
    );
  } else {
...
/**
 * Extract machine-readable fields from STATE.md markdown body and build a YAML frontmatter object.
 * Allows hooks and scripts to read state reliably via `state json` instead of fragile regex
 * parsing.
 */
function buildStateFrontmatter(bodyContent, cwd) {
  ...
  const stoppedAt =
    stateExtractField(sessionBodyScope, "Stopped At") ||
    stateExtractField(sessionBodyScope, "Stopped at");
...
/** Acquire a lockfile for STATE.md operations. Returns the lock path for later release. */
...
/**
 * Write STATE.md with synchronized YAML frontmatter. All STATE.md writes should use this instead of
 * raw writeFileSync. Uses a simple lockfile to prevent parallel agents from overwriting each
 * other's changes (race condition with read-modify-write cycle).
 */
...
/**
 * Atomic read-modify-write for STATE.md. Holds the lock across the entire read -> transform ->
 * write cycle, preventing the lost-update problem where two agents read the same content and the
...
/**
 * Update STATE.md when a new phase begins execution. Updates body text fields (Current focus,
 * Status, Last Activity, Current Position) and synchronizes frontmatter via writeStateMd. Fixes:
 * #1102 (plan counts), #1103 (status/last_activity), #1104 (body text).
 */
...
/**
 * Write a WAITING.json signal file when GSD hits a decision point. External watchers (fswatch,
 * polling, orchestrators) can detect this. File is written to .planning/WAITING.json (or
 * .gsd/WAITING.json if .gsd exists). Fixes #1034.
 */
...
/**
 * Update the ## Performance Metrics section in STATE.md content. Increments Velocity totals and
 * upserts a By Phase table row. Returns modified content string.
 */
...
/**
 * Gate 3a: Record state after plan-phase completes. Updates Status to "Ready to execute", Total
 * Plans, Last Activity.
 */
...
/**
 * Bug #2630: reset STATE.md for a new milestone cycle. Stomps frontmatter
 * milestone/milestone_name/status/progress AND rewrites the Current Position body. Preserves
 * Accumulated Context. Symmetric with the SDK `stateMilestoneSwitch` handler.
 */
...
/** Gate 1: Validate STATE.md against filesystem. Returns { valid, warnings, drift } JSON. */
...
/**
 * Gate 2: Sync STATE.md from filesystem ground truth. Scans phase dirs, reconstructs counters,
 * progress, metrics. Supports --verify for dry-run mode.
 */
...
/**
 * Prune old entries from STATE.md sections that grow unboundedly (#1970). Moves decisions,
 * recently-completed summaries, and resolved blockers older than keepRecent phases to
 * STATE-ARCHIVE.md.
 *
 * Options: keepRecent: number of recent phases to retain (default: 3) dryRun: if true, return what
 * would be pruned without modifying STATE.md
 */
function cmdStatePrune(cwd, options, raw) {
  ...
  function prunePass(content) {
    ...
    const blockersPattern =
      /(###?\s*(?:Blockers|Blockers\/Concerns|Blockers\s*&\s*Concerns)\s*\n)([\s\S]*?)(?=\n###?|\n##[^#]|$)/i;
...
/**
 * Mark the current phase as COMPLETE in STATE.md. Updates Status, Last Activity, and the Current
 * Position section to reflect that the phase execution is finished and the project is ready for the
 * next phase. Implements the `gsd state complete-phase` subcommand (issue #2735).
 */
function cmdStateCompletePhase(cwd, raw) {
  ...
  readModifyWriteStateMd(
    ...
    (content) => {
      ...
      const rawPhase =
        stateExtractField(content, "Current Phase") || stateExtractField(content, "Phase") || "";
