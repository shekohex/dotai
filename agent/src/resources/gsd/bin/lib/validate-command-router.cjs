...
function routeValidateCommand({ verify, args, cwd, raw, parseNamedArgs, output, error }) {
  ...
  if (subcommand === "consistency") {
    ...
    const RECOMMENDATIONS = {
      [STATES.HEALTHY]: null,
      [STATES.WARNING]:
        "Context is approaching the fracture zone — consider /gsd-thread to continue in a fresh window.",
      [STATES.CRITICAL]:
        "Reasoning quality may degrade past 70% utilization (fracture point). Run /gsd-thread now to preserve output quality.",
    };
    let classified;
    try {
      classified = classifyContextUtilization(
        Number(opts["tokens-used"]),
        Number(opts["context-window"]),
      );
    } catch (e) {
