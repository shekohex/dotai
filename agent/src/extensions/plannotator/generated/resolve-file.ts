 ...
 *
 * Resolves a user-provided path to an absolute file path using three strategies: 1. Exact path
 * (absolute or relative to cwd) 2. Case-insensitive relative path search within project root 3.
 * Case-insensitive bare filename search within project root
 *
...
import { CODE_FILE_REGEX as CODE_FILE_BASENAME_REGEX } from "./code-file.js";
export { CODE_FILE_REGEX, isCodeFilePath } from "./code-file.js";
...
export function normalizeUserPathInput(input: string, platform = process.platform): string {
...
function isAbsoluteNormalizedUserPath(input: string, platform = process.platform): boolean {
...
export function isAbsoluteUserPath(input: string, platform = process.platform): boolean {
...
/**
 * Trigger (or return the in-flight) walk of `projectRoot` for code files. Cached for
 * `FILE_LIST_CACHE_TTL_MS`. Storing a Promise (not a value) makes concurrent callers piggyback on
 * the same walk — first arrival wins.
 *
 ...
 * 3. If `baseDir` provided, literal `<baseDir>/<input>` existence check — lets out-of-tree linked docs
 *    resolve their own relative references (e.g. `../script.ts` in `~/notes/foo.md` finds
 *    `~/script.ts`).
 * 4. Case-insensitive suffix match against the cached file list:
 *
 *    - Bare basename input → match any file with that basename;
 *    - Input with `/` → match files whose path equals or ends with `/<input>` on a segment boundary (so
 *      `editor/App.tsx` matches `packages/editor/App.tsx` but not `myeditor/App.tsx`).
 *
 * `..` segments in the input are honored: only `./` is stripped before suffix matching. `../foo.ts`
 * without a `baseDir` correctly falls through to not_found rather than fabricating a match against
 * `foo.ts` somewhere in cwd.
 */
 ...
 */
function resolveMarkdownFileCore(input: string, projectRoot: string): ResolveResult {
 ...
 * @param projectRoot - Project root directory to search within
 */
export function resolveMarkdownFile(input: string, projectRoot: string): ResolveResult {
  const originalInput = input.trim();
  const unquotedInput = stripWrappingQuotes(originalInput);
...
/**
 * Check if a directory contains at least one file matching the given extensions. Used to validate
 * folder annotation targets.
 *
