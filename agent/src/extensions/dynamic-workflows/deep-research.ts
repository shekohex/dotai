/** Deep research workflow. Built-in workflow for comprehensive research across multiple sources. */
 ...
 *
 * The script is static and reads its inputs from `args` (question/angles/minSupport), so the
 * question is never string-interpolated into source — no escaping hazards. Enable websearch at run
 * time via the agent's `toolNames` option.
 *
...
export function generateCodebaseAuditWorkflow(scope: string, checks: string[]): string {
  const escapedScope = scope.replaceAll("'", "\\'").slice(0, 60);
