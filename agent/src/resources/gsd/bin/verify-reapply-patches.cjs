...
/**
 * Deterministic verifier for the /gsd-reapply-patches Step 5 "Hunk Verification Gate". For each
 * backed-up patch file, asserts that the user's added lines (computed from a real diff against the
 * pristine baseline, not from the LLM's prose summary) survive into the merged output.
 *
 * Usage:
 ...
 *
 * Exit codes: 0 — every user-added line is present in the merged file (gate passes) 1 — at least
 * one missing line in at least one file (gate fails) 2 — usage / structural error (e.g. patches dir
 * missing)
 *
 * Bug #2969: the Step 5 gate previously trusted Claude's free-text "verified: yes/no" reporting per
 * hunk. The LLM was filling in `yes` even when content had been silently dropped. Moving the check
 * to a deterministic script is the durability fix.
 */
...
/**
 * Compute the set of "user-added" lines: lines present in the backup but absent from the pristine
 * baseline. If no pristine is provided, falls back to using every significant line in the backup
 * (over-broad but safe — favours false-positive failures over silent successes, which is the right
 * side to err on for #2969).
 */
...
/**
 * Stable reason codes for the per-file result. Tests assert via `assert.equal(result.reason,
 * REASON.X)` rather than regex-matching prose, so the diagnostic surface is a typed enum, not free
 * text.
 *
 * Adding a new reason requires updating the REASON map AND the tests' shape assertion that locks
 * the documented set of codes.
 */
...
function main() {
  ...
  if (opts.json) {
    process.stdout.write(
      JSON.stringify({ checked: results.length, failures: failures.length, results }, null, 2) +
        "\n",
    );
  } else {
