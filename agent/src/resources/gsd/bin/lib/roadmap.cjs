...
/**
 * Coerce an arbitrary YAML scalar/object into a string for cross-cutting truth aggregation.
 * Handles:
 *
 * - Strings (passthrough)
 * - Numbers / booleans (String() coercion — issue #2770: bare YAML ints like `- 3` must be surfaced,
 *   not silently skipped)
 * - Kv-shaped objects from parseMustHavesBlock continuation kv (issue #2757) — extract the first
 *   meaningful string field
 *
 * Returns the empty string when no usable text can be derived; callers should skip empty results.
 */
...
/**
 * Search for a phase header (and its section) within the given content string. Returns a result
 * object if found (either a full match or a malformed_roadmap checklist-only match), or null if the
 * phase is not present at all.
 */
...
/**
 * Annotate the ROADMAP.md plan list for a phase with wave dependency notes and a cross-cutting
 * constraints subsection derived from PLAN frontmatter.
 *
 * Wave dependency notes: "Wave 2 — blocked on Wave 1 completion" inserted as bold headers before
 * each wave group in the plan checklist.
 *
 * Cross-cutting constraints: must_haves.truths strings that appear in 2+ plans are surfaced in a
 * "Cross-cutting constraints" subsection below the plan list.
 *
 * The operation is idempotent: if wave headers already exist in the section the function returns
...
function cmdRoadmapAnnotateDependencies(cwd, phaseNum, raw) {
  ...
  if (!phaseInfo || phaseInfo.plans.length === 0) {
    output(
      { updated: false, reason: "no plans found for phase", phase: phaseNum },
      raw,
      "no plans",
    );
    return;
  ...
  output(
    ...
    raw,
    updated
      ? `annotated ${waves.length} wave(s), ${crossCuttingTruths.length} constraint(s)`
      : "skipped (already annotated or no plan list)",
  );
}
