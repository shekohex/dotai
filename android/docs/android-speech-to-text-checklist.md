...
- VAD/live transcription review: compared against Google LiteRT `MicrophoneAudioSource.kt`, which zeroes samples below `SILENCE_THRESHOLD` but keeps yielding fixed overlapping windows, and VoiceInk streaming lifecycle, which keeps recording active until explicit stop while partial text updates continue only from meaningful audio. Found and fixed a loophole where Android marked `speechDetected` forever after first speech and kept scheduling partial transcription through long silence. VAD now exposes `voiceActive` and `speechPaused`, zeroes below-threshold silent frames before buffering, never finalizes automatically on trailing silence, and partial transcription scheduling pauses after configured trailing silence until voice resumes.
