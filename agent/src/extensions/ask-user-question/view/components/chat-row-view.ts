...
/**
 * Per-tick projection of chat-row state. The chat row is a single-item `WrappingSelect` rendered in
 * the question-tab footer; it owns no per-tab state — only `focused` (whether the chat row is the
 * active focus target) and `numbering` (display number aligned with the active tab's items).
 */
...
/**
 * Typed wrapper around the chat-row `WrappingSelect`. Replaces the prior raw-primitive consumption
 * at `props-adapter.ts:106, :120` and removes the accidental surface area (8 unused
 * `WrappingSelect` setters) noted in research Q4.
 *
 * Pattern modeled after `OptionListView` (`option-list-view.ts:27-93`): mirror-then-delegate
 * `setProps`; render is pure delegation; `Component` triplet forwards.
 */
