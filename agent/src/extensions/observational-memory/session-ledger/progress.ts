...
export function entryIndexForId(entries: Entry[], entryId: string | undefined): number {
  if (entryId === undefined || entryId.length === 0) return -1;
...
/**
 * Return whichever coverage marker sits earlier on the branch.
 *
 * @param {Entry[]} entries Current branch entries.
 * @param {string | undefined} firstId First coverage marker id.
 * @param {string | undefined} secondId Second coverage marker id.
 * @returns {string | undefined} The earlier marker, or undefined when either is unknown.
 */
export function earlierCoverageMarkerId(
  ...
  if (firstId === undefined) return secondId;
  if (secondId === undefined) return firstId;
...
/**
 * Estimated raw/source tokens observed since the latest observation coverage marker.
 *
 * @param {Entry[]} entries Current branch entries.
 * @returns {number} Estimated uncovered raw tokens.
 */
...
/**
 * Estimated raw/source tokens since the latest reflection coverage marker.
 *
 * @param {Entry[]} entries Current branch entries.
 * @returns {number} Estimated uncovered raw tokens.
 */
...
/**
 * Read a valid assistant-reported context token count from a message entry.
 *
 * @param {Entry} entry Candidate branch entry.
 * @returns {number | undefined} Provider-reported context tokens when valid.
 */
 ...
 *
 * Only usage from an assistant that responded AFTER the compaction is a valid post-compaction
 * baseline: pi's own docs state the last assistant usage before/at a compaction reflects the
 * PRE-compaction context size. The usage carried on the compaction entry itself is the
 * summary-generation call's usage (pre-compaction scale, a different LLM call), so it is
 * deliberately NOT used as a baseline.
 */
/**
 * Real context tokens right after a compaction anchor.
 *
 * @param {Entry[]} entries Current branch entries.
 * @param {number} compactionIdx Index of the compaction boundary entry.
 * @returns {number | undefined} Post-compaction baseline tokens when measurable.
 */
export function realContextTokensAfterCompaction(
  entries: Entry[],
...
/**
 * Real context tokens at the time observation coverage ended: last valid assistant usage at/before
 * the covered entry. Returns undefined when no valid usage exists (e.g. an error/abort storm) —
 * callers must fall back to the raw estimate rather than measuring from zero, which would otherwise
 * read the full context as "growth" and re-fire stages every turn.
 */
/**
 * Real context tokens at the time observation coverage ended.
 *
 * @param {Entry[]} entries Current branch entries.
 * @param {number} coverageIdx Index of the coverage marker entry.
 * @returns {number | undefined} Baseline tokens when measurable.
 */
export function realContextTokensAtCoverage(
  entries: Entry[],
  coverageIdx: number,
): number | undefined {
...
/**
 * Real context growth since the most recent anchor (a compaction, or the given coverage marker),
 * measured from provider-reported usage.
 *
 * Returns undefined when the baseline cannot be measured reliably — no usage at/after the anchor,
 * or the current context is SMALLER than the baseline (accounting basis changed, e.g. a mid-session
 * model/provider switch that counts usage differently). Callers must fall back to the raw estimate
 * in that case; clamping a stale baseline to 0 would starve the stage forever, and measuring from
 * zero would over-fire it.
 */
/**
 * Real context growth since the most recent anchor.
 *
 * @param {Entry[]} entries Current branch entries.
 * @param {V3MemoryCustomType | undefined} customType Anchor ledger type, when known.
 * @param {number} currentContextTokens Current provider-reported context tokens.
 * @returns {number | undefined} Measured growth, or undefined when unmeasurable.
 */
export function realTokensSinceAnchor(
  ...
  const coverageIdx = customType === undefined ? -1 : latestCoverageIndex(entries, customType);
