...
The main call surface is a single SwiftUI `Window` configured through a lifecycle-aware window accessor as a structurally titleless floating AppKit window, so native traffic-light controls cannot be recreated during window or workspace transitions; Settings remains a normal macOS window. The window keeps the resizable style capability because AppKit allows those windows to become key without requiring a title bar, preserving focused Space and Escape handling. The coordinator reasserts this presentation during SwiftUI updates and display monitoring without activating Pi Live. The native rectangular window shadow is disabled so the connected state reads as a true floating orb. SwiftUI owns the intentional orb and glass glow. Window discovery no longer depends on title matching, `NotificationCenter`, or delayed launch positioning.
