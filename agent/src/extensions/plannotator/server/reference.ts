 ...
 * VaultNode, buildFileTree, walkMarkdownFiles, handleDocRequest, detectObsidianVaults,
 * handleObsidian*, handleFileBrowserRequest
 */
 ...
 * @param {string[]} results Output accumulator.
 * @param {RegExp} extensions Allowed extensions pattern.
 */
...
export async function handleDocRequest(res: Res, url: URL): Promise<void> {
  ...
  const resolvedBase = base !== null && base.length > 0 ? resolveUserPath(base) : null;
  if (
    resolvedBase !== null &&
...
/**
 * Batch existence check for code-file paths the renderer wants to linkify. POST /api/doc/exists
 * with { paths: string[] }.
 *
 * @param {Res} res Response object.
...
export async function handleDocExistsRequest(res: Res, req: IncomingMessage): Promise<void> {
  ...
  const baseDir =
    typeof baseRaw === "string" && baseRaw.length > 0 ? resolveUserPath(baseRaw) : undefined;
...
export function handleObsidianDocRequest(res: Res, url: URL): void {
  ...
  if (vaultPath === null || vaultPath.length === 0 || filePath === null || filePath.length === 0) {
