...
- Read milestone history from `.planning/milestones/` and current planning docs (what shipped previously)
- Read STATE.md (pending todos, blockers)
- Check for MILESTONE-CONTEXT.md (from `/gsd discuss-milestone` if available in caller workflow)
...
- Parse last version from `.planning/milestones/` and current planning state
...
**If "Adjust":** Ask what needs changing (plain text, not another structured interview). Incorporate changes, re-present the summary. Loop until "Looks good" is selected.
...
Use template: {{GSD_BUNDLE_DIR}}/templates/research-project/{FILE}
</output>
...
> **ORCHESTRATOR RULE — CODEX RUNTIME**: After starting all 4 researcher subagents above, do NOT read research files or synthesize content independently while the subagents are active. Wait for all 4 researchers to complete before spawning the synthesizer. This prevents duplicate work and wasted context.
...
> **ORCHESTRATOR RULE — CODEX RUNTIME**: After starting the synthesizer subagent above, stop working on this task immediately. Do not read more files, edit code, or run tests related to this task while the subagent is active. Wait for the subagent to return its result. This prevents duplicate work, conflicting edits, and wasted context. Only resume when the subagent result is available.
...
- .planning/milestones/ (archive history)
</files_to_read>
...
> **ORCHESTRATOR RULE — CODEX RUNTIME**: After starting the roadmapper subagent above, stop working on this task immediately. Do not read more files, edit code, or run tests related to this task while the subagent is active. Wait for the subagent to return its result. This prevents duplicate work, conflicting edits, and wasted context. Only resume when the subagent result is available.
