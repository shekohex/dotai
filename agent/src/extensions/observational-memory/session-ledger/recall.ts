...
export function recallMemorySources(entries: Entry[], memoryId: string): RecallResult {
  ...
  let kind: "mixed" | "reflection" | "observation";
  if (directObservationMatches.length > 0 && reflectionMatches.length > 0) kind = "mixed";
  else if (reflectionMatches.length === 0) kind = "observation";
