...
/**
 * Match pending todos against a phase's goal/name/requirements. Returns todos with relevance scores
 * based on keyword, area, and file overlap. Used by discuss-phase to surface relevant todos before
 * scope-setting.
 */
...
/**
 * Check whether a commit should be allowed based on commit_docs config. When commit_docs is false,
 * rejects commits that stage .planning/ files. Intended for use as a pre-commit hook guard.
 */
