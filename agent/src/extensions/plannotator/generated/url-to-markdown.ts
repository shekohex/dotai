 ...
 *
 * Fetches a URL via Jina Reader (default) or plain fetch + Turndown, returning clean markdown for
...
/**
 * True when the source indicates the markdown was converted from HTML, not returned as-is from the
 * origin.
 */
 ...
 *
 * IMPORTANT — IPv6 hostname format (verified empirically in Bun 1.3.11 and Node 22): The WHATWG URL
 * `hostname` getter returns IPv6 addresses WITH brackets. This is why PRIVATE_IPV6 uses `^\[` — it
 * matches the actual runtime output.
 *
 * Verified outputs (both Bun and Node return identical results): new
 * URL("http://[::1]:3000/").hostname → "[::1]" new URL("http://[fe80::1]/").hostname → "[fe80::1]"
 * new URL("http://[fc00::1]/").hostname → "[fc00::1]" new URL("http://[fd12::1]/").hostname →
 * "[fd12::1]" new URL("http://[::ffff:192.168.0.1]/").hostname → "[::ffff:c0a8:1]" new
 ...
 *
 * When `useJina` is true, attempts Jina Reader first (returns markdown directly, handles
 * JS-rendered pages). On failure, warns to stderr and falls back to plain fetch + Turndown.
 */
...
/**
 * Fetch a URL as raw text — for .md/.mdx URLs that are already markdown. Returns null if the server
 * returns HTML (e.g. GitHub's viewer page for a .md file), signaling the caller to fall through to
 * Jina/Turndown.
 *
 * Uses redirect: "manual" with isLocalUrl validation on each hop — same SSRF protection as
 * fetchViaTurndown.
 */
async function fetchRawText(url: string): Promise<string | null> {
  ...
  try {
    ...
    if (err instanceof Error && err.name === "AbortError") {
      throw new Error(`Timed out fetching ${url}`, { cause: err });
    }
    throw err;
  } finally {
    clearTimeout(timer);
...
/**
 * Content negotiation fast path — request `text/markdown` via the Accept header. Sites that support
 * Cloudflare's "Markdown for Agents" (or similar) will return markdown directly, letting us skip
 * Jina and Turndown entirely. Returns null if the server doesn't serve markdown.
 */
...
async function fetchViaJina(url: string): Promise<string> {
  ...
  try {
    ...
    if (err instanceof Error && err.name === "AbortError") {
      throw new Error("timed out", { cause: err });
    }
    throw err;
  } finally {
    clearTimeout(timer);
