 ...
 *
 * Claude has its own review model (severity-based findings with reasoning traces) separate from
 * Codex's priority-based model. The transform layer normalizes both into the shared annotation
 * format.
 *
 * Claude uses --json-schema (inline JSON + Ajv validation with retries) and --output-format
 * stream-json for live JSONL streaming. The final event is type:"result" with structured_output
 * containing validated findings.
 */
...
/**
 * Build the `claude -p` command. Prompt is passed via stdin, not as a positional arg — avoids
 * quoting issues, argv limits, and variadic flag conflicts.
 */
...
/**
 * Parse Claude Code's stream-json output (JSONL). Extracts structured_output from the final
 * type:"result" event.
 */
...
/**
 * Extract log-worthy content from a JSONL line for the LiveLogViewer. Returns a human-readable
 * string, or null if the line should be skipped.
 */
  ...
  try {
    ...
    if (event.type === "assistant" && event.message?.content) {
      const parts = Array.isArray(event.message.content)
        ...
        : [event.message.content];
      const texts = parts.filter((p: any) => p.type === "text" && p.text).map((p: any) => p.text);
