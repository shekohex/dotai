 ...
 *
 * SUMMARIZATION BATCH (Ph1 step 2): One batch = one completed assistant turn with tool calls,
 * captured from the `turn_end` event when event.toolResults.length > 0. event.message =
 * AssistantMessage (contains ToolCall content blocks with ids) event.toolResults =
 * ToolResultMessage[] (one per tool call in this turn)
 *
 * STATE MODEL (Ph1 step 3):
 *
 * - Runtime state: Map<toolCallId, ToolCallRecord> rebuilt on session_start
 * - Session metadata: pi.appendEntry("context-prune-index", IndexEntryData) stored once per
 *   summarized batch; NOT in LLM context
 * - User config: .pi/settings.json → "contextPrune" key (JSON merge safe, Pi preserves unknown keys
 *   when rewriting settings files)
 *
 * CONFIG FORMAT: { "contextPrune": { "enabled": true, "summarizerModels": ["provider/model"],
 * "showPruneStatusLine": true } } summarizerModels: ["default"] = use current active model
 * (ctx.model). "provider/model-id" = explicit model via ctx.modelRegistry.find()
 *
 * SUMMARY MESSAGE FORMAT (Ph1 step 5): customType: "context-prune-summary" content: markdown with
 * one bullet per tool call + short-id footer details: SummaryMessageDetails (toolCallRefs,
 * toolNames, turnIndex, timestamp) The content itself includes short alias IDs in plain text so the
 * model can reference them in future context_tree_query calls without needing details.
 *
 * API CONSTRAINTS (Ph1 step 6):
 *
 * - Pruning MUST happen in the `context` event via { messages: filtered }, never by mutating session
 *   history (pi.appendEntry / session file untouched)
 * - Summary injection uses pi.sendMessage(..., { deliverAs: "steer" }) from inside the turn_end
 *   handler so it lands before the next LLM call
 * - Original full tool outputs are preserved in IndexEntryData (session custom entries) and
 *   accessible via context_tree_query at any time
 * - V1 prunes only ToolResultMessage entries; the AssistantMessage tool-call blocks (which carry the
 *   toolCallIds) are intentionally kept so the model can still reference them when calling
 *   context_tree_query
 * - "default" summarizer = ctx.model (current active model + its credentials), NOT a hidden
 *   side-channel. It makes an explicit LLM call from turn_end.
 */
 ...
 * - "agent-message" : batches up turns and flushes when the agent sends a final text response (a turn
 *   with no tool calls), or when the agent loop ends (default)
 * - "agentic-auto" : the LLM agent decides when to prune by calling the context_prune tool; the tool
 *   is only active in this mode and guided by prompt instructions
 */
export type PruneOn =
  | "every-turn"
  | "on-context-tag"
...
/**
 * Granularity of pruning batches. - "turn" : one summary per assistant turn (default; current
 * behavior) - "agent-message" : one summary per full user → final-agent-message span (merges all
 * turns between two consecutive user messages)
 */
...
/** Extension config stored under `contextPrune` in ~/.pi/agent/settings.json */
export interface ContextPruneConfig {
  ...
  pruneOn: PruneOn;
  /**
   * Whether to inject a small ephemeral reminder before each LLM call telling the model how many
   * unpruned tool-call results have piled up. Only honored when `enabled && pruneOn ===
   * "agentic-auto"`. In all other modes this flag is a no-op (the reminder is meant to nudge the
   * LLM to call `context_prune` at a sensible cadence).
   */
  remindUnprunedCount: boolean;
  /**
   * Granularity of each pruning batch. - "turn" : one summary per assistant turn (default) -
   * "agent-message" : one summary per user → final-agent-message span (all turns between two user
   * messages are merged)
   */
...
/**
 * One complete batch from a single turn_end event. Represents one assistant turn that contained
 * tool calls.
 */
export interface CapturedBatch {
  ...
  /**
   * Grouping key assigned by `captureUnindexedBatchesFromSession`. Increments for each user message
   * seen while walking the branch. Batches from the live `turn_end` path do NOT have this field set
   * (they are always emitted one-per-turn regardless of batchingMode). Used by `groupBatchesByMode`
   * to merge turns within the same user → agent-message span when batchingMode ===
   * "agent-message".
   */
...
/**
 * A single tool call record stored in the runtime index. Contains the full original tool output for
...
/**
 * Data stored via pi.appendEntry(CUSTOM_TYPE_INDEX, data). One entry per summarized batch;
 * reconstructed into the runtime index on session_start.
 */
...
/**
 * Short alias used in the summary message text plus the real toolCallId it maps back to for future
 * recovery through context_tree_query.
 */
...
/**
 * Details stored in the custom summary message's `details` field. Machine-readable metadata so
 * renderers and extensions can inspect summaries.
 */
...
/**
 * Cumulative token/cost stats for summarizer LLM calls. Persisted via
 * pi.appendEntry(CUSTOM_TYPE_STATS, ...) so stats survive restarts and branch navigation.
 */
 ...
 *
 * This advances both when pruning succeeds and when a summary is rejected for being larger than the
 * raw tool-result text it would replace. Operational failures do not advance the frontier.
 */
...
/**
 * Progress callback invoked by `flushPending` when processing batches sequentially. Only fired when
 * the caller passes `onProgress` in `FlushOptions` (i.e. `/pruner now`).
 */
...
export interface FlushOptions {
  /**
   * Delivery path: "runtime" uses sendMessage/steer (default); "session" writes directly to
   * session.
   */
  delivery?: "runtime" | "session";
  /** Optional one-shot batching override for programmatic integrations. */
  batchingMode?: BatchingMode;
  /**
   * When provided, batches are processed sequentially (one LLM call each) instead of in parallel,
   * and this callback is invoked before/after each batch. Used by `/pruner now` to drive the
   * multi-row progress overlay.
   */
  onProgress?: ProgressCallback;
  /**
   * When provided, receives the number of summary characters streamed so far for the
   * currently-running batch. Used by `/pruner now` to show live progress.
   */
  onBatchTextProgress?: BatchTextProgressCallback;
  /**
   * Pre-captured batches from a prior `capturePendingBatches()` call. When set, `flushPending`
   * skips the internal capture step and uses these directly. Avoids double-capture when the caller
   * needs to know the batch count before opening the progress overlay.
   */
  previewedBatches?: CapturedBatch[];
  /**
   * Abort signal — when fired the in-flight summarization is cancelled and `flushPending` returns
   * `{ ok: false, reason: "aborted" }` without advancing the frontier. All pending batches are
   * restored so the next flush can retry.
   */
  signal?: AbortSignal;
}
...
export interface SummarizeBatchesOptions {
  ...
  onBatchTextProgress?: BatchTextProgressCallback;
  /**
   * Abort signal forwarded to every individual summarizeBatch() call. When fired, all in-flight
   * stream calls are cancelled.
   */
  signal?: AbortSignal;
}
