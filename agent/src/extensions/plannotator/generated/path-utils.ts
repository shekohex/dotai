...
/**
 * Strip a cwd prefix from an absolute path to get a repo-relative path. Used by review agent
 * transforms to convert absolute file paths from agent output into diff-compatible relative paths.
 *
 * Uses path.relative for cross-platform support (Windows backslashes) and normalizes to forward
 * slashes for git diff path matching.
 */
