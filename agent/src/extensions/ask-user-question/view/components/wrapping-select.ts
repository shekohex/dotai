...
/**
 * Row-intent discriminated union. `kind` is the single discriminator — pre-1.0.3 boolean flags have
 * been removed (see `banned-flags.test.ts`). Modeled after `QuestionnaireAction`
 * (`key-router.ts:13-32`) and `Effect` (`state-reducer.ts:26-32`) — pure literal-tagged variants,
 * no shared base, exhaustive-`switch` enforcement via non-`void` returns.
 *
 * Variant semantics: - `option`: a regular author-defined option row. - `other`: the inline
 * free-text input row appended to single-select questions (label is "Type something."). Renders as
 * inline `Input` when active. - `chat`: the abandon-questionnaire escape-hatch row (label is "Chat
 * about this"). - `next`: the explicit commit-and-advance row appended to multi-select questions
 ...
 * Numbering controls.
 *
 * Use `numberStartOffset` + `totalItemsForNumbering` when a list is logically a slice of a larger
 * numbered sequence — e.g. the chat row lives in its own WrappingSelect but should render as
 * `(N+1).` where N is the previous list's item count, with the column padded as if both lists were
 * one continuous numbered sequence.
 */
...
export class WrappingSelect implements Component {
  ...
  /**
   * Index of the row that was previously confirmed for this list (e.g. the user's prior answer when
   * re-entering a multi-question tab). Renders `<label> ✔` in the active-row styling but WITHOUT
   * the `❯` pointer — pointer is reserved for the live cursor. When `selectedIndex ===
   * confirmedIndex && focused`, the active rendering wins (no double-mark).
   */
  private confirmedIndex: number | undefined = undefined;
  /**
   * When set together with `confirmedIndex`, replaces the row's static label at render time. Used
   * for the `kind: "other"` sentinel — its label is "Type something." but if the user's prior
  ...
  /**
   * Update the numbering offset + total padding width without rebuilding the component. Used by the
   * host to keep the chat-row WrappingSelect's number aligned with the active tab's options list
   * when the user switches tabs (each tab can have a different items count).
   *
   * @param {number} numberStartOffset - Starting offset for numbering
   * @param {number} totalItemsForNumbering - Total items for column padding
   */
  ...
  /**
   * Mark a previously-confirmed row. Pass `undefined` to clear. `labelOverride` replaces the row's
   * static `item.label` at render time — used for the `kind: "other"` sentinel so the row reads
   * `Hello ✔` instead of `Type something. ✔` when the prior answer was custom text.
   *
   * @param {number | undefined} index - Index to mark confirmed, or undefined to clear
   * @param {string} [labelOverride] - Optional label override for confirmed row
   */
  ...
  /** @param {string} text - Input buffer text */
   ...
   *
   * @param {number | undefined} offset - Cursor offset or undefined
   */
   ...
   *
   * @param {string} _data - Input data (unused)
   */
  ...
  /**
   * Returns the [startRow, endRow) range of the focused (selected) item within the output of
   * `render(width)`. Computed by iterating the visible window and summing per-item row counts —
   * O(maxVisible) per call.
   *
   * @param {number} width - Rendering width
   * @returns {[number, number]} [start, end) row range
   */
  ...
  /**
   * Per-item row count. Delegates to `renderItem().length` so `renderItem` remains the single
   * source of truth for per-item row math — eliminates the prior shadow-copy that risked silent
   * miscounts when new `kind` values branch in `renderItem` but not here.
   *
   * @param {WrappingSelectItem} item - Item to count rows for
   * @param {number} index - Item index
   * @param {boolean} isActive - Whether item is active
   * @param {number} width - Rendering width
   * @param {number} numberWidth - Number column width
   * @returns {number} Row count for this item
   */
  ...
  /**
   * Render the inline input row across one or more lines, wrapping at `contentWidth` so long input
   * doesn't run off the right edge or trip the parent renderer's width invariant. Mirrors
   * `renderLabelBlock`'s contract: first line carries `rowPrefix`, continuation lines carry
   * `continuationPrefix` (spaces), and every emitted line passes through `theme.selectedText`.
   *
   * Cursor visualization follows the standard TUI input-widget pattern (ECMA-48 SGR 7 reverse-video
   * on the cell _at_ the cursor, not an inserted glyph) — same approach used by pi-tui Input.render
   * (input.js:411-418), ink-text-input, terkelg/prompts, ratatui's user-input example, etc. Split:
   * `before | at | after` where `at` is the single character under the cursor (or U+00A0 NBSP at
   * end-of- buffer) wrapped in `\x1b[7m…\x1b[27m`. No characters shift; the column under the cursor
   * inverts.
   *
   * Pi-tui's zero-width `CURSOR_MARKER` (APC sentinel) is emitted immediately before the `at` cell
   * so the TUI framebuffer can position the hardware terminal cursor at that column (visible iff
   * pi's `showHardwareCursor` setting is on — `pi-coding-agent/main.js:303`). `visibleWidth` strips
   * the marker as zero-width (utils.js:187-203), so wrap math is preserved.
   *
   * NBSP at end-of-buffer (rather than literal space): `wrapTextWithAnsi` tokenizes on whitespace,
   * so a literal space inside the reverse-video escape pair would cause `wrapTextWithAnsi` to break
   * the line at the cursor. NBSP preserves the visible width-1 contribution without registering as
   * a wrap break — the conventional workaround documented across ANSI-aware string wrappers.
   *
   * @param {string} rowPrefix - Row prefix string
   * @param {string} continuationPrefix - Continuation indent
   * @param {number} contentWidth - Content area width
   * @returns {string[]} Rendered input lines
   */
  private renderInlineInputRow(
    ...
    const raw = `${before}${CURSOR_MARKER}\u001B[7m${atCursor}\u001B[27m${after}`;
   ...
   * @param {string} continuationPrefix - Continuation indent
   * @param {number} contentWidth - Content area width
   * @param {boolean} applySelectedStyle - Whether to apply selected styling
   * @returns {string[]} Rendered label lines
   */
