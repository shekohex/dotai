 ...
 *
 * Detects structural drift between a committed codebase and the `.planning/codebase/STRUCTURE.md`
 * map produced by `gsd-codebase-mapper`.
 *
 * Four categories of drift element:
 *
 * - New_dir → a newly-added file whose directory prefix does not appear in STRUCTURE.md
 * - Barrel → a newly-added barrel export at (packages|apps)/<name>/src/index.(ts|tsx|js|mjs|cjs)
 * - Migration → a newly-added migration file under one of the recognized migration directories
 *   (supabase, prisma, drizzle, src/migrations, …)
 * - Route → a newly-added route module under a `routes/` or `api/` dir
 *
 * Each file is counted at most once; when a file matches multiple categories the most specific
 * category wins (migration > route > barrel > new_dir).
 *
 * Design decisions (see PR for full rubber-duck):
 *
 * - The library is pure. It takes parsed git diff output and returns a structured result. The
 *   CLI/workflow layer is responsible for running git and for spawning mappers.
 * - `last_mapped_commit` is stored as YAML-style frontmatter at the top of each
 *   `.planning/codebase/*.md` file. This keeps the baseline attached to the file, survives git
 *   moves, and avoids a sidecar JSON.
 * - The detector NEVER throws on malformed input — it returns a `{ skipped: true }` result. The phase
 *   workflow depends on this non-blocking guarantee.
 */
...
/**
 * True iff any prefix of `file` (dir1, dir1/dir2, …) appears as a substring of `structureMd`. Used
 * to decide whether a file is in "mapped territory".
 *
 * Matching is deliberately substring-based — STRUCTURE.md is free-form markdown, not a structured
 * manifest. If the map mentions `src/lib/` the check `structureMd.includes('src/lib')` holds.
 */
 ...
 * @param {number} [input.threshold=3] - Min number of drift elements that triggers action. Default
 *   is `3`
 * @param {"warn" | "auto-remap"} [input.action='warn'] Default is `'warn'`
...
/**
 * Filter `paths` to only those that are safe to splice into a mapper prompt. Any path that is
 * absolute, contains traversal, or includes shell metacharacters is dropped.
 */
...
/**
 * Read `last_mapped_commit` from the frontmatter of a `.planning/codebase/*.md` file. Returns null
 * if the file does not exist or has no frontmatter.
 */
...
/**
 * Upsert `last_mapped_commit` and `last_mapped_at` into the frontmatter of the given file,
 * preserving any other frontmatter keys and the body.
 */
