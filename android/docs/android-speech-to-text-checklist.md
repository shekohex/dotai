...
- Warm-cache correction: keep-model-warm is now enabled by default for this personal app. Warm creates and retains the LiteRT `CompiledModel`, encode/decode tensor buffers, TDT decoder buffers, and tokenizer for the remembered `LiteRtParakeetTranscriber` until the chat input composable is disposed or model/accelerator settings change. This removes compile/tokenizer setup from tap-to-first-token when model/tokenizer are ready; actual GPU/NPU residency is determined by LiteRT delegate registration and Pixel logs still need to confirm which accelerator was accepted.
