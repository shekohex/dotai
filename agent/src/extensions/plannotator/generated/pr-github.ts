...
import { encodeApiFilePath } from "./pr-provider.js";
...
export async function fetchGhPR(
  ...
  const metadata: PRMetadata = {
    ...
    defaultBranch:
      repoResult.exitCode === 0 && repoResult.stdout.trim() && repoResult.stdout.trim() !== "null"
...
function parseGhPRContext(raw: Record<string, unknown>): PRContext {
  ...
  return {
    ...
    linkedIssues: arr(raw.closingIssuesReferences).map((i: any) => ({
      ...
      url: str(i?.url),
      repo: i?.repository ? `${login(i.repository.owner)}/${str(i.repository.name)}` : "",
    })),
  };
}
...
async function fetchGhReviewThreads(runtime: PRRuntime, ref: GhPRRef): Promise<PRReviewThread[]> {
  const result = await runtime.runCommand(
    "gh",
    hostnameArgs(ref.host, [
      "api",
...
export async function fetchGhPRFileContent(
  ...
  const cleaned = base64Content.replaceAll(/\n/g, "");
  try {
...
/**
 * Fetch the per-file "viewed" state for a GitHub PR via GraphQL. Returns a map of { filePath:
 * isViewed } where isViewed is true for VIEWED or DISMISSED states (i.e., the file was reviewed but
 * may need re-review after new commits).
 */
export async function fetchGhPRViewedFiles(
  ...
  do {
    ...
    for (const node of files.nodes) {
      ...
      result[node.path] =
        node.viewerViewedState === "VIEWED" || node.viewerViewedState === "DISMISSED";
    }
...
/**
 * Mark or unmark a set of files as viewed in a GitHub PR via GraphQL mutations. Uses
 * Promise.allSettled so a single file failure doesn't block the rest. Throws only if ALL mutations
 * fail.
 */
...
/**
 * Walk up and down the PR stack from the current PR, resolving PR numbers/titles for every node in
 * the chain.
 *
 * Up: walk from currentPR.baseBranch → defaultBranch (ancestors) Down: walk from
 * currentPR.headBranch → leaf PRs (descendants)
 */
...
export async function fetchGhPRList(runtime: PRRuntime, ref: GhPRRef): Promise<PRListItem[]> {
  const result = await runtime.runCommand("gh", [
    "pr",
    "list",
    "--repo",
