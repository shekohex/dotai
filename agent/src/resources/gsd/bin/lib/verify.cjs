...
/**
 * Validate agent installation status (#1371). Returns detailed information about which agents are
...
/**
 * Detect structural drift between the committed tree and `.planning/codebase/STRUCTURE.md`.
 * Non-blocking: any failure returns a `{ skipped: true }` JSON result with a reason; the command
 * never exits non-zero so `execute-phase`'s drift gate cannot fail the phase.
 */
function cmdVerifyCodebaseDrift(cwd, raw) {
  ...
  try {
    ...
    const config = loadConfig(cwd);
    const threshold =
      Number.isInteger(config?.workflow?.drift_threshold) && config.workflow.drift_threshold >= 1
