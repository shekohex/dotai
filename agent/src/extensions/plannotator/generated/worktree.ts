 ...
 *
 * Uses ReviewGitRuntime so both Bun and Node runtimes can use the same logic. Lives in
 * packages/shared/ and gets vendored to Pi via vendor.sh.
 *
 * Designed as composable primitives, not tied to any specific use case. PR local checkout, agent
 * sandboxes, parallel sessions — all compose from these.
 */

import type { ReviewGitRuntime } from "./review-core.js";
...
/** Fetch a ref from origin. Runs: `git fetch origin <ref>` Throws on failure. */
...
/**
 * Ensure a git object (commit SHA) is available locally. Checks with `git cat-file -t`, fetches
 * from origin if missing. Returns true if the object is available after the attempt.
 */
...
/**
 * Create a git worktree. Runs: `git worktree add [--detach] <path> <ref>` Throws on failure with a
 * descriptive error.
 */
...
/**
 * Remove a git worktree. Best-effort — logs errors but does not throw. Runs: `git worktree remove
 * [--force] <path>`
 */
