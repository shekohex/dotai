 ...
 *
 * Enumerates the file names that gsd workflows officially produce at the .planning/ root level.
 * Used by gsd-health (W019) to flag unrecognized files so stale or misnamed artifacts don't
 * silently mislead agents or reviewers.
 *
...
/**
 * Return true if `filename` (basename only, no path) matches a canonical .planning/ root artifact —
 * either an exact name or a known pattern.
 *
