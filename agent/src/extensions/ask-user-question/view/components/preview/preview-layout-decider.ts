...
/**
 * VisibleWidth(" ✔") = 2 (space + ✔ codepoint). Reserved on the longest-label measurement so a
 * confirmed row never gets truncated when MIN_LEFT clamps the column.
 */
 ...
 *
 * The terminal-width gate is the AND check from the previous `preview-pane.ts` — lifted here so the
 * decision is computed ONCE per render and threaded explicitly through `previewBlockHeight`.
 * Removes the bug class where `previewBlockHeight` re-derived `sideBySide` from a column width
 * (already < pane width post-split), capping height too short.
 */
/**
 * @param {number} terminalWidth - Terminal width
 * @param {number} paneWidth - Pane width
 * @returns {PreviewLayoutMode} Layout mode
 */
...
/**
 * Compute the adaptive left column width from option labels. Pure function — deterministic for a
 * given (items, totalForNumbering, paneWidth).
 *
 * Pipeline:
 *
 ...
 */
/**
 * @param {readonly WrappingSelectItem[]} items - Option items
 * @param {number} totalForNumbering - Total items for number padding
 * @param {number} paneWidth - Pane width
 * @returns {number} Adaptive left column width
 */
export function adaptiveLeftWidth(
  ...
  /* visibleWidth(" ✔") = 2 (space + ✔ codepoint) */
...
/**
 * Cross-tab maximum left-column width. Aggregates `adaptiveLeftWidth` over every tab and returns
 * the widest result, so the options column stays stable on tab switch.
 *
 * Pure function — `tabs.length` MUST equal `itemsByTab.length`. Multi-select tabs use
 * `items.length` for numbering; single-select tabs add 1 for the chat row slot. Floor is `MIN_LEFT`
 * so an all-empty input still produces a usable column.
 */
/**
 * @param {ReadonlyArray<{ multiSelect?: boolean }>} tabs - Tab configurations
 * @param {ReadonlyArray<readonly WrappingSelectItem[]>} itemsByTab - Items per tab
 * @param {number} paneWidth - Pane width
 * @returns {number} Cross-tab max left width
 */
export function crossTabMaxLeftWidth(
  ...
  for (let i = 0; i < tabs.length; i++) {
    ...
    const totalForNumbering = tabs[i]?.multiSelect === true ? items.length : items.length + 1;
...
/**
 * Source-line probe: measures the widest source line across all options' previews. Returns 0 when
 * no option carries a preview.
 *
 * V1 heuristic — works well for tree/table/heading/list content where source-line width ≈ rendered
 * width. Paragraphs overstate (source width "wants" the full pane); the worst case is "donation
 * does nothing useful" — fallback to the label-driven path. Upgrade path: replace with a
 * render-width probe that invokes Markdown at a probe width.
 *
 * Pure function — O(total chars) per question; no Markdown invocation, no cache interaction.
 */
/**
 * @param {QuestionData} question - Question data
 * @returns {number} Widest preview source line width
 */
...
/**
 * Cross-tab/cross-option preview budget. Iterates all questions, computes each question's preview
 * appetite via `previewSourceWidth`, adds box + padding overhead (5 cols), and returns the widest
 * result. Floor is `MIN_PREVIEW_WIDTH` so a previewless question still reserves a usable column.
 * Ceiling ensures the left column retains its `MIN_LEFT` floor.
 *
 * Pure function — same cross-tab max pattern as `crossTabMaxLeftWidth`.
 */
/**
 * @param {readonly QuestionData[]} questions - All questions
 * @param {number} paneWidth - Pane width
 * @returns {number} Cross-tab preview budget
 */
...
/**
 * Cross-tab left-column width with slack donation. Combines the label-driven width (from
 * `crossTabMaxLeftWidth`) with the slack donated by narrow previews.
 *
 * Pipeline:
 *
 ...
 * 4. Return `min(max(labelDriven, slackDonation), ceiling)` where `ceiling = paneWidth − GAP −
 *    MIN_PREVIEW_WIDTH`
 *
 * Invariants:
 *
 ...
 *
 * `crossTabMaxLeftWidth` is NOT replaced — it continues to exist as the primitive. `MAX_LEFT_RATIO`
 * caps the label-driven path inside `adaptiveLeftWidth`; donation operates above the cap when
 * preview slack is available.
 */
/**
 * @param {ReadonlyArray<{ multiSelect?: boolean }>} tabs - Tab configurations
 * @param {ReadonlyArray<readonly WrappingSelectItem[]>} itemsByTab - Items per tab
 * @param {readonly QuestionData[]} questions - All questions
 * @param {number} paneWidth - Pane width
 * @returns {number} Left width with donation
 */
...
/**
 * Width allocation for side-by-side mode. `adaptiveLeft` is the pre-computed left column width
 * (from `adaptiveLeftWidth`, cross-tab aggregated). The Math.max(1, ...) calls keep both columns >=
 * 1 col on extreme inputs.
 */
/**
 * @param {number} paneWidth - Pane width
 * @param {number} adaptiveLeft - Adaptive left column width
 * @returns {{ leftWidth: number; rightWidth: number; gap: number }} Column widths and gap
 */
...
/**
 * Returns the widths actually passed to `options.render` and `previewLines` inside `render()`.
 * Stacked uses the full pane width for both; side-by-side splits via `columnWidths`, with the
 * preview column offset by `PREVIEW_PADDING_LEFT`.
 */
/**
 * @param {number} paneWidth - Pane width
 * @param {PreviewLayoutMode} mode - Layout mode
 * @param {number} adaptiveLeft - Adaptive left width
 * @returns {{ optionsWidth: number; previewWidth: number }} Body widths for options and preview
 */
