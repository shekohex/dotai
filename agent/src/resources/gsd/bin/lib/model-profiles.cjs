 ...
 *
 * Lets users tune model selection at the _phase-type_ level (planning, research, execution,
 * verification, ...) instead of per-agent. Maps each agent in MODEL_PROFILES to exactly one
 * phase-type so resolution is deterministic.
 *
 ...
 * - Discuss: collaborative scoping (no subagent today; reserved for orchestrators that may spawn one)
...
/**
 * The six phase-type slots accepted in `.planning/config.json` `models` block. `discuss` and
 * `completion` are reserved — no current agent maps to them today — so users can pre-configure
 * those slots without breaking validation when an orchestrator starts honoring them.
 */
 ...
 *
 * Each agent declares a default routing tier (light/standard/heavy) that the dynamic-routing
 * resolver uses to pick from `dynamic_routing.tier_models[tier]` on the first attempt. On
 * orchestrator-detected soft failure, the resolver escalates to the next tier up (capped at
 * `max_escalations`).
 *
 * Tier semantics:
 *
...
/**
 * The three valid agent tier slots for dynamic routing. Used to validate
 * `dynamic_routing.tier_models.<tier>` keys at config-set time and the AGENT_DEFAULT_TIERS values
 * at startup.
 */
const VALID_AGENT_TIERS = new Set(["light", "standard", "heavy"]);

/**
 * Tier escalation order: light → standard → heavy. `nextTier(currentTier)` returns the tier one
 * step up. `heavy` stays at heavy (no tier above). Returns null for invalid input so callers can
 * detect mis-config rather than silently degrade.
 */
...
function getAgentToModelMapForProfile(normalizedProfile) {
  ...
  for (const [agent, profileToModelMap] of Object.entries(MODEL_PROFILES)) {
    agentToModelMap[agent] =
      normalizedProfile === "inherit" ? "inherit" : profileToModelMap[normalizedProfile];
  }
