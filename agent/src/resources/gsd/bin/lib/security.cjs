 ...
 *
 * This module centralizes security checks for GSD tooling. Because GSD generates markdown files
 * that become LLM system prompts (agent instructions, workflow state, phase plans), any
 * user-controlled text that flows into these files is a potential indirect prompt injection
 * vector.
 *
 * Threat model: 1. Path traversal: user-supplied file paths escape the project directory 2. Prompt
 * injection: malicious text in arguments/PRDs embeds LLM instructions 3. Shell metacharacter
 * injection: user text interpreted by shell 4. JSON injection: malformed JSON crashes or corrupts
 * state 5. Regex DoS: crafted input causes catastrophic backtracking
 */
"use strict";
...
/**
 * Validate that a file path resolves within an allowed base directory. Prevents path traversal
 * attacks via ../ sequences, symlinks, or absolute paths.
 *
 * @param {string} filePath - The user-supplied file path
 * @param {string} baseDir - The allowed base directory (e.g., project root)
 * @param {object} [opts] - Options
 * @param {boolean} [opts.allowAbsolute=false] - Allow absolute paths (still must be within
 *   baseDir). Default is `false`
...
/**
 * Validate a file path and throw on traversal attempt. Convenience wrapper around validatePath for
 * use in CLI commands.
 */
...
/**
 * Patterns that indicate prompt injection attempts in user-supplied text. These patterns catch
 * common indirect prompt injection techniques where an attacker embeds LLM instructions in text
 * that will be read by an agent.
 *
 * Note: This is defense-in-depth — not a complete solution. The primary defense is proper
 * input/output boundaries in agent prompts.
 */
...
/**
 * Layer 2: Encoding-obfuscation patterns with custom finding messages. Each entry: { pattern:
...
/**
 * Scan text for potential prompt injection patterns. Returns an array of findings (empty = clean).
 *
 * @param {string} text - The text to scan
 * @param {object} [opts] - Options
 * @param {boolean} [opts.strict=false] - Enable stricter matching (more false positives). Default
 *   is `false`
...
/**
 * Sanitize text that will be embedded in agent prompts or planning documents. Strips known
 * injection markers while preserving legitimate content.
 *
 * This does NOT alter user intent — it neutralizes control characters and instruction-mimicking
 * patterns that could hijack agent behavior.
 *
...
function sanitizeForPrompt(text) {
  ...
  sanitized = sanitized.replace(
    /\[(\/?)(SYSTEM|INST)\]/gi,
    (_, slash, tag) => `[${slash}${tag.toUpperCase()}-TEXT]`,
  );
...
/**
 * Sanitize text that will be displayed back to the user. Removes protocol-like leak markers that
 * should never surface in checkpoints.
 *
 * @param {string} text - Text to sanitize
 * @returns {string} Sanitized text
 */
...
/**
 * Validate that a string is safe to use as a shell argument when quoted. This is a defense-in-depth
 * check — callers should always use array-based exec (spawnSync) where possible.
 *
...
/**
 * Safely parse JSON with error handling and optional size limits. Wraps JSON.parse to prevent
 * uncaught exceptions from malformed input.
 *
 * @param {string} text - JSON string to parse
 * @param {object} [opts] - Options
 * @param {number} [opts.maxLength=1048576] - Maximum input length (1MB default). Default is
 *   `1048576`
 * @param {string} [opts.label='JSON'] - Description for error messages. Default is `'JSON'`
...
/**
 * Validate a phase number argument. Phase numbers must match: integer, decimal (2.1), or letter
 * suffix (12A). Rejects arbitrary strings that could be used for injection.
 *
...
/**
 * Validate a STATE.md field name to prevent injection into regex patterns. Field names must be
 * alphanumeric with spaces, hyphens, underscores, or dots.
 *
...
/**
 * Validate the XML structure of a prompt file. For agent/workflow files, flags any XML tag not in
 * the known-valid set.
 *
