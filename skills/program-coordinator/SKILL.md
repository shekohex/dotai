...
2. Run `scripts/coordinator_state.py summary --product-id <id>`. Initialize only when state does not exist.
3. Load only active initiatives, recent events, pending approvals, active leases, open PRs, deployed heads, and knowledge relevant to current work.
...
5. Acquire product coordinator lease before mutation. One canonical coordinator host and one writer per product.
