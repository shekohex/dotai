 ...
 *
 * Reads Claude Code session history (read-only) to extract user messages for behavioral profiling.
 * Three commands: - scan-sessions: list all projects and sessions - extract-messages: extract user
 * messages from a specific project - profile-sample: multi-project sampling with recency weighting
 */
