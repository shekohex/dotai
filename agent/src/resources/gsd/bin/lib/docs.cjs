 ...
 *
 * Provides `cmdDocsInit` which returns project signals, existing doc inventory with GSD marker
 * detection, doc tooling detection, monorepo awareness, and model resolution. Used by Phase 2 to
 * route doc generation appropriately.
 */
...
/**
 * Check whether a file begins with the GSD doc writer marker. Reads the first 500 bytes only —
 * avoids loading large files.
 *
 ...
 * for Markdown files, excluding dirs in SKIP_DIRS.
 *
 * @param {string} cwd - Project root
 * @returns {{ path: string; has_gsd_marker: boolean }[]}
 */
...
/**
 * Detect project type signals from the filesystem and package.json. All checks are best-effort and
 * never throw.
 *
 * @param {string} cwd - Project root
...
/**
 * Extract monorepo workspace globs from pnpm-workspace.yaml, package.json workspaces, or
 * lerna.json.
 *
 * @param {string} cwd - Project root
...
/**
 * Return JSON context for the docs-update workflow: project signals, existing doc inventory, doc
 * tooling detection, monorepo workspaces, and model resolution. Follows the cmdInitMapCodebase
 * pattern.
 *
 * @example
