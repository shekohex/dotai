...
/**
 * Returns `undefined` so inline input rendering falls back to end-of-buffer cursor. Follow-up:
 * replace with Input.getCursorOffset() when pi-tui exposes public cursor API.
 *
 * @param {Input} input - Pi-tui Input instance
 * @returns {number | undefined} Cursor offset or undefined
 */
...
export interface QuestionnairePropsAdapterConfig {
  ...
  /**
   * Renderables not reached by the binding registries (e.g. the notes `Input`, which is typed into
   * directly and has no props). Walked by `invalidate()` after the binding-driven components.
   */
  extraInvalidatables?: ReadonlyArray<Invalidatable>;
}

/**
 * View fan-out: drives every component setter from the canonical state via two binding registries.
 * `globalBindings` covers the cross-tab components (chatRow, dialog, submitPicker?, tabBar?);
 * `perTabBindings` covers the per-tab kinds (optionList, preview, multiSelect?). The hand-coded
 * fan-out collapses to one global loop + one nested per-tab loop. The inline-Other value is read
 * from the headless `inlineInput` instance per tick into ctx so `selectOptionListProps` sees the
 * live value.
 */
export class QuestionnairePropsAdapter {
  ...
  /**
   * Invalidates every owned renderable. Called by the session in place of the old
   * `dialog.invalidate()` forwarding chain — DialogView no longer reaches into siblings (chatRow,
   * tabBar, notesInput, activePreviewPane). Iterates the same registries used by `apply()` plus
