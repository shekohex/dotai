...
function foldProjection(entries: Entry[], options: ProjectionFoldOptions): Projection {
  ...
  const observationsBoundary = resolveBoundaryIndex(entries, indexes, options.observationsBoundary);
  const reflectionsBoundary = resolveBoundaryIndex(entries, indexes, options.reflectionsBoundary);
  const dropsBoundary = resolveBoundaryIndex(entries, indexes, options.dropsBoundary);
...
export function fullProjection(entries: Entry[], upToEntryId?: string): Projection {
  const boundary = upToEntryId === undefined ? tipBoundary() : entryBoundary(upToEntryId);
...
export function visibleProjection(entries: Entry[], upToEntryId?: string): Projection {
  if (upToEntryId === undefined) {
    ...
    return details === undefined
      ...
      : projectionFromMemoryDetails(details);
  }
...
export function latestFullFoldBoundaryId(entries: Entry[]): string | undefined {
  ...
  for (let i = entries.length - 1; i >= 0; i--) {
    ...
    if (entry.firstKeptEntryId === undefined || entry.firstKeptEntryId.length === 0) continue;
...
export function buildCompactionProjection(
  ...
  const maintenanceBoundary =
    fullFoldBoundaryId === undefined ? noneBoundary() : entryBoundary(fullFoldBoundaryId);
