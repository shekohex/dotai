...
export type QuestionnaireAction =
  ...
  | { kind: "focus_chat" }
  /**
   * Carries the target index so UP/DOWN form a continuous cycle through `[chat, option0, …,
   * optionLast]`.
   */
  | { kind: "focus_options"; optionIndex: number }
  | { kind: "notes_forward"; data: string }
  /**
   * Flip `state.collapsed`. Always available, regardless of inner mode (see top intercept in
   * `routeKey`).
   */
...
function buildSingleSelectAnswer(
  ...
  const q = runtime.questions.at(state.currentTab);
  if (!q) return null;
...
export function routeKey(
  ...
  if (data === NOTES_ACTIVATE_KEY && q.multiSelect !== true && state.focusedOptionHasPreview) {
  ...
  if (q.multiSelect === true) {
    ...
    if (data === SPACE_KEY) {
      if (focusedMeta?.blocksMultiToggle === true) return { kind: "ignore" };
    ...
    if (kb.matches(data, KEYBIND_CONFIRM)) {
      ...
      if (focusedMeta?.autoSubmitsInMulti !== true)
        return { kind: "toggle", index: state.optionIndex };
