...
/**
 * Declarative side-effects emitted by `reduce`. The runtime executes them after committing the new
 * state, then asks the props-adapter to re-project. Closed set — adding an effect requires updating
 * both the union AND the runtime's `runEffect` switch (compiler-enforced exhaustive). No
 * string-keyed escape hatch.
 */
export type Effect =
...
function syncMultiSelectFromAnswers(
  ...
  if (q?.multiSelect !== true) return new Set();
...
function persistMultiSelectAnswer(
  ...
  if (q?.multiSelect !== true) return state.answers;
  ...
  out.set(state.currentTab, {
    ...
    answer: null,
    selected,
    ...(pendingNotes !== undefined && pendingNotes.length > 0 ? { notes: pendingNotes } : {}),
  });
  return out;
}
...
/**
 * Per-kind handler signature: action payload narrows to the matching union member via `Extract`, so
 * handlers consume fully-typed actions without `as` casts.
 */
...
const navHandler: Handler<"nav"> = (state, action, ctx) => {
  ...
  const inputMode = item === undefined ? false : ROW_INTENT_META[item.kind].activatesInputMode;
...
const multiConfirmHandler: Handler<"multi_confirm"> = (state, action, ctx) => {
  const q = ctx.questions[state.currentTab];
  if (q === undefined) return { state, effects: [] };
  const pendingNotes = state.notesByTab.get(state.currentTab);
...
const notesExitHandler: Handler<"notes_exit"> = (state, _action, _ctx) => {
  ...
  if (trimmed.length === 0) {
    ...
    if (prev?.notes !== undefined && prev.notes.length > 0) {
      const stripped = { ...prev };
...
const focusOptionsHandler: Handler<"focus_options"> = (state, action, ctx) => {
  ...
  const inputMode =
    focused === undefined ? false : ROW_INTENT_META[focused.kind].activatesInputMode;
  const next = withFocusedOptionHasPreview(
    {
      ...state,
      chatFocused: false,
...
/**
 * Compile-time-exhaustive dispatch table. `{ [K in Kind]: Handler<K> }` requires an entry per union
 * member — adding a new `QuestionnaireAction` variant fails to compile here until a handler is
 * registered, mirroring the `Record<RowKind, …>` pattern used by `ROW_INTENT_META`.
 */
const _HANDLERS: { [K in QuestionnaireAction["kind"]]: Handler<K> } = {
  nav: navHandler,
...
/**
 * Pure reducer: (state, action, ctx) → (state, Effect[]). Mirrors `rpiv-todo`'s
 * `applyTaskMutation`. Delegates to `HANDLERS` — per-kind handlers above are pure, named, and
 * individually testable. `ignore` is also handled outside the reducer by `handleIgnoreInline` in
 * the runtime fast path.
 *
 * @param {QuestionnaireState} state - Current questionnaire state.
 * @param {QuestionnaireAction} action - Action to apply.
 * @param {ApplyContext} ctx - Session-lifetime context.
 * @returns {ApplyResult} New state and effects.
 */
export function reduce(
  ...
  switch (kind) {
    case "nav":
      return navHandler(state, action, ctx);
    case "tab_switch":
      return tabSwitchHandler(state, action, ctx);
    case "confirm":
      return confirmHandler(state, action, ctx);
    case "toggle":
      return toggleHandler(state, action, ctx);
    case "multi_confirm":
      return multiConfirmHandler(state, action, ctx);
    case "cancel":
      return cancelHandler(state, action, ctx);
    case "notes_enter":
      return notesEnterHandler(state, action, ctx);
    case "notes_exit":
      return notesExitHandler(state, action, ctx);
    case "notes_forward":
      return notesForwardHandler(state, action, ctx);
    case "submit":
      return submitHandler(state, action, ctx);
    case "submit_nav":
      return submitNavHandler(state, action, ctx);
    case "focus_chat":
      return focusChatHandler(state, action, ctx);
    case "focus_options":
      return focusOptionsHandler(state, action, ctx);
    case "toggle_collapsed":
      return toggleCollapsedHandler(state, action, ctx);
    case "ignore":
      return ignoreHandler(state, action, ctx);
    default:
      return ignoreHandler(state, action, ctx);
  }
}
