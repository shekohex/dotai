 ...
 *
 * All functions use the `glab` CLI via the PRRuntime abstraction. Self-hosted instances are
 * supported via the --hostname flag.
 */

import type { PRRuntime, PRMetadata, PRContext, PRReviewFileComment } from "./pr-provider.js";
 ...
 *
 * Each entry has: { diff, old_path, new_path, new_file, deleted_file, renamed_file } We construct
 * proper `diff --git` headers that the UI parser expects.
 */
...
export async function fetchGlMRContext(runtime: PRRuntime, ref: GlMRRef): Promise<PRContext> {
  ...
  const isDraft =
    mr.draft === true || (typeof mr.title === "string" && /^(Draft:|WIP:)/i.test(mr.title));
