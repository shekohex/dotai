...
/**
 * Check whether graphify is enabled in the project config. Reads config.json directly via fs.
 * Returns false by default (when no config, no graphify key, or on error).
 *
 ...
 * @param {{ timeout?: number }} [options={}] - Options (timeout in ms, default 30000). Default is
 *   `{}`
 * @returns {{ exitCode: number; stdout: string; stderr: string }}
 */
/**
 * Frozen enum of typed reason codes for execGraphify failures (#2974). Tests assert on
...
/**
 * Check whether the graphify CLI binary is installed and accessible on PATH. Uses --help (NOT
 * --version, which graphify does not support).
 *
...
/**
 * Detect graphify version and check compatibility. Tested range: >=0.4.0,<1.0
 *
 * Detection strategy:
 *
...
function checkGraphifyVersion() {
  ...
  if (!versionStr) {
    const pyResult = childProcess.spawnSync(
      "python3",
...
/**
 * Safely read and parse a JSON file. Returns null on missing file or parse error. Prevents crashes
 * on malformed JSON (T-02-01 mitigation).
 *
...
/**
 * Build a bidirectional adjacency map from graph nodes and edges. Each node ID maps to an array of
 * { target, edge } entries. Bidirectional: both source->target and target->source are added
 * (Pitfall 3).
 *
 * @param {{ nodes: object[]; edges: object[] }} graph
 * @returns {Object<string, { target: string; edge: object }[]>}
 */
...
/**
 * Seed-then-expand query: find nodes matching term, then BFS-expand up to maxHops. Matches on node
 * label and description (case-insensitive substring, D-01).
 *
 * @param {{ nodes: object[]; edges: object[] }} graph
 * @param {string} term - Search term
 * @param {number} [maxHops=2] - Maximum BFS hops from seed nodes. Default is `2`
...
/**
 * Apply token budget by dropping edges by confidence tier (D-04, D-05, D-06). Token estimation:
 * Math.ceil(JSON.stringify(obj).length / 4). Drop order: AMBIGUOUS -> INFERRED -> EXTRACTED.
 *
...
/**
 * Query the knowledge graph for nodes matching a term, with optional budget cap. Uses
 * seed-then-expand BFS traversal (D-01).
 *
 * @param {string} cwd - Working directory
 * @param {string} term - Search term
 * @param {{ budget?: number | null }} [options={}] Default is `{}`
 * @returns {object}
 */
...
/**
 * Pre-flight checks for graphify build (BUILD-01, BUILD-02, D-09). Does NOT invoke graphify --
 * returns structured JSON for the builder agent.
 *
 * @param {string} cwd - Working directory
 * @returns {object}
 */
...
/**
 * Write a diff snapshot after successful build (D-06). Reads graph.json from .planning/graphs/ and
 * writes .last-build-snapshot.json using atomicWriteFileSync for crash safety.
 *
 * @param {string} cwd - Working directory
 * @returns {object}
 */
