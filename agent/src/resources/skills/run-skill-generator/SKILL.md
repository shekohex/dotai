...
Your job is to produce a **skill** at `<unit>/.pi/skills/run-<unit-name>/`
...
```
<unit>/.pi/skills/run-<unit>/
  SKILL.md         ← SHORT. Points at the driver. Has the frontmatter
                     that lets Pi auto-load it when someone asks
...
([../run-app/examples/playwright.md](../run-app/examples/playwright.md)) — and the skill is
the script that runs it. For a desktop app
([../run-app/examples/electron.md](../run-app/examples/electron.md)), the driver is a custom
...
The skill lives at `<unit>/.pi/skills/run-<unit-name>/`, where
`<unit>` is the directory for **one deployable thing** — an app, a
service, a library.

Pi discovers bundled and project skills from `.pi/skills/` and configured skill directories:
an agent working anywhere inside `<unit>` will see
...
- **Single-project repo:** `.pi/skills/run-<repo-name>/` at repo root.
- **Large repo with many apps:** one per app, colocated —
  `apps/billing/.pi/skills/run-billing/`,
  `apps/desktop/.pi/skills/run-desktop/`.
...
List the project's skills with their descriptions (same probe `run-app`
...
(Also check for a legacy `.pi/run.md` — earlier versions of this
...
| ---------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| "Requires macOS/Windows"                 | Launch it on Linux anyway. Apps rarely refuse to start — they crash on a missing `.so`, which `apt-get` fixes. Native modules for _your host's_ keychain/notifications may no-op; the core usually runs. |
...
| "Run `npm start`"                        | That's the human path (spawns a window, waits forever). Find or build the _programmatic_ path — `electron-forge start` to build then launch via Playwright, or equivalent.                               |
...
- **Build a harness to drive it.** You need a handle on the running
  ...
  **Cover the layer(s) PRs actually touch.** A tmux driver that pokes
  ...
  you don't write it (see [../run-app/examples/playwright.md](../run-app/examples/playwright.md)).
...
(`/run-billing`). The `description:` is what Pi scans to decide
...
`.pi/skills/run-<unit-name>/driver.mjs` — it's long, but explicit.
...
the `run-app` skill (same per-project-type patterns are used as the
...
| -------------------------- | ------------------------------------------------------------------------ | ---------------------------------------------------------------------- |
| Web server / API           | Background-launch + `curl`-based smoke script                            | [../run-app/examples/server.md](../run-app/examples/server.md)         |
| CLI tool                   | Representative-args smoke script, check exit codes + output              | [../run-app/examples/cli.md](../run-app/examples/cli.md)               |
| TUI / interactive terminal | tmux wrapper: `send-keys` / `capture-pane`                               | [../run-app/examples/tui.md](../run-app/examples/tui.md)               |
| Electron / desktop GUI     | Playwright `_electron` REPL driver under xvfb, screenshots, tmux-wrapped | [../run-app/examples/electron.md](../run-app/examples/electron.md)     |
| Browser-driven             | dev server + `chromium-cli` script                                       | [../run-app/examples/playwright.md](../run-app/examples/playwright.md) |
| Library / SDK              | Import-and-call smoke script                                             | [../run-app/examples/library.md](../run-app/examples/library.md)       |

For a web app, start from [../run-app/examples/playwright.md](../run-app/examples/playwright.md)
— drive it with `chromium-cli`, no custom driver needed. For a
desktop app, start from [../run-app/examples/electron.md](../run-app/examples/electron.md)
