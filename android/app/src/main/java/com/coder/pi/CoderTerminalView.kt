...
data class TerminalOscMetadata(
    val title: String,
    val pwd: String,
    val bellCount: Long,
)

data class TerminalNotificationContext(
...
data class TerminalGestureAction(
    val id: String,
    val label: String,
    val description: String,
)

data class TerminalAccessibleLine(
    val row: Int,
    val text: String,
)
...
const val TerminalNotificationUrlKey = "notification_url"

class CoderTerminalView
    @JvmOverloads
    constructor(
        context: Context,
        attrs: AttributeSet? = null,
        attachedEngine: TerminalEngine? = null,
        ...
        override fun onSurfaceCreated(
            ...
            native.nativeRendererSetShaderCacheDir(
                rendererHandle,
                context.cacheDir
                    .resolve("shader-cache")
                    .apply { mkdirs() }
                    .absolutePath,
            )
        ...
        override fun onCheckIsTextEditor(): Boolean = softwareKeyboardAllowed
            ...
            when (event.actionMasked) {
                ...
                MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
                    ...
                    if (mouseTrackingTouch) {
                        if (mouseTouchMoved) {
                            endSmoothScrollGesture()
                        } else if (event.actionMasked == MotionEvent.ACTION_UP && openHyperlinkAt(event.x, event.y)) {
                            endSmoothScrollGesture()
                        } else {
        ...
        fun sendText(text: String) {
            ...
            if (terminalTextInputUsesUtf8(output, prefix.isNotEmpty(), remoteInput != null)) {
                writeInput(prefix + output.toByteArray(Charsets.UTF_8))
            } else {
                native.nativeTextInput(handle, output)
            }
            shiftLatch = false
            altLatch = false
            notifyModifierLatchChanged()
        ...
        fun terminalMouseTrackingActive(): Boolean = handle != 0L && remoteInput != null && native.nativeMouseTracking(handle)
        ...
        private inner class TerminalAccessibilityHelper : ExploreByTouchHelper(this) {
            ...
            override fun onPopulateNodeForVirtualView(
                ...
                val text =
                    lines
                        .firstOrNull { it.row == virtualViewId }
                        ?.text
                        .orEmpty()
                        .ifBlank { "Terminal has no visible text" }
                node.text = text
        ...
        fun fontSizePoints(): Int = fontSizeSp.coerceIn(8, 32)
        ...
        fun shortcutRowActive(
            tabId: String,
            shortcut: ShortcutRowDefinition,
            defaultActive: Boolean,
        ): Boolean = preferences.getBoolean(shortcutRowPreferenceKey(tabId, shortcut), defaultActive)

        fun shortcutRowOrder(
            tabId: String,
            shortcuts: List<ShortcutRowDefinition>,
        ): List<String> = normalizeShortcutRowOrder(preferences.getString("shortcuts.row.$tabId.order", null), shortcuts)

        fun setShortcutRowOrder(
            tabId: String,
            order: List<String>,
            shortcuts: List<ShortcutRowDefinition>,
        ) {
        ...
        fun selectedGestureAction(
            gesture: String,
            defaultAction: String,
        ): String = preferences.getString("gesture.action.$gesture", defaultAction).orEmpty().ifBlank { defaultAction }
        ...
        fun customShortcuts(): List<TerminalShortcut> =
            preferences.getString("toolbar.shortcuts", "").orEmpty().split("\n").mapNotNull { line ->
        ...
        fun replaceCustomShortcut(
            oldShortcut: ShortcutRowDefinition,
            newShortcut: TerminalShortcut,
        ) {
            ...
            when (event.eventName) {
                "agent.alert" ->
                    snapshot.alerts
                        ...
                        ?.let { handlePiAgentNotification(it) }
        ...
        private fun handleOscClipboard(
            kind: String,
            data: String,
        ) {
        ...
        private fun handleOscNotification(
            title: String,
            body: String,
        ) {
        ...
        private fun handlePiAgentNotification(notification: TerminalAgentNotificationPresentation) {
            val message = listOf(notification.title, notification.body).filter { it.isNotBlank() }.joinToString(" · ").take(256)
            if (message.isBlank()) return
            val now = System.currentTimeMillis()
            if (now - lastNotificationMillis < 3000L) return
            lastNotificationMillis = now
        ...
        private fun handlePiAgentProgress(progress: TerminalAgentProgressPresentation) {
            if (!oscNotificationsEnabled() || !oscNotificationProgressEnabled()) return
            if (!progress.active) {
                stopPiAgentProgressStatusUpdates()
        ...
        private fun postOscProgressNotification(
            title: String,
            body: String,
            state: Int,
            progress: Int,
        ...
        private fun postPiAgentProgressNotification(
            title: String,
            body: String,
            elapsedSeconds: Long?,
        ): Boolean {
        ...
        @SuppressLint("NewApi")
        private fun postNativePiAgentProgressNotification(
            notificationId: Int,
            title: String,
            body: String,
            elapsedSeconds: Long?,
            ...
            val builder =
                android.app.Notification
                    ...
                    .addAction(
                        ...
                        android.app.Notification.Action
                            .Builder(Icon.createWithResource(context, R.drawable.ic_feather_message_circle), "Follow up", replyPendingIntent)
                            .addRemoteInput(
                                android.app.RemoteInput
                                    .Builder(TerminalNotificationReplyInputKey)
                                    .setLabel("Follow up")
                                    .build(),
        ...
        @SuppressLint("NewApi")
        private fun postNativeProgressNotification(
            title: String,
            body: String,
            state: Int,
            progress: Int,
            ...
            val channelId = oscProgressNotificationChannelId()
            val launchIntent = terminalNotificationLaunchIntent()
            val pendingIntent = PendingIntent.getActivity(context, oscProgressNotificationId(), launchIntent, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
            val notificationBody = body.ifBlank { title }.take(512)
            val style =
                android.app.Notification
                    .ProgressStyle()
                    ...
                    .setProgressSegments(
                        listOf(
                            android.app.Notification.ProgressStyle
                                .Segment(100)
                                .setColor(if (state == 2) Color.RED else Color.rgb(125, 92, 255)),
                        ),
                    )
            val builder =
                android.app.Notification
                    ...
                    .addAction(
                        android.app.Notification.Action
                            .Builder(oscNotificationIconRes(), "Open terminal", pendingIntent)
                            .build(),
                    )
            builder.setLargeIcon(workspaceNotificationIcon())
        ...
        private fun postOscNotification(
            title: String,
            body: String,
            ongoing: Boolean,
            progress: Int,
            indeterminate: Boolean,
            notificationId: Int = if (ongoing) oscProgressNotificationId() else nextTerminalNotificationId(),
        ...
        private fun copyUrlNotificationAction(
            notificationId: Int,
            url: String,
        ...
        private fun ensureOscProgressNotificationChannel() {
            ...
            if (notificationManager.getNotificationChannel(channelId) == null) {
                notificationManager.createNotificationChannel(
                    NotificationChannel(channelId, oscProgressNotificationChannelName(), NotificationManager.IMPORTANCE_DEFAULT).apply {
                        setSound(null, null)
        ...
        private fun piAgentProgressNotificationId(): Int = (oscProgressNotificationId() xor 0x50694167) and 0x7fffffff
        ...
        private fun oscNotificationIconRes(): Int =
            when (oscNotificationIconStyle()) {
...
fun terminalOscHyperlinkHost(value: String): String? = terminalOscHyperlinkUri(value)?.host?.lowercase()
...
fun terminalFontPixelSize(
    context: Context,
    fontSizeSp: Int,
): Int = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, fontSizeSp.coerceIn(8, 32).toFloat(), context.resources.displayMetrics).roundToInt().coerceAtLeast(8)

fun terminalCellHeightForFontSize(
    context: Context,
    fontSizeSp: Int,
    key: String = CoderFonts.selectedKey(context),
): Int {
    val paint =
...
fun terminalCellWidthForFontSize(
    context: Context,
    fontSizeSp: Int,
    key: String = CoderFonts.selectedKey(context),
): Int {
...
private fun terminalMetricTypeface(
    context: Context,
    key: String,
): Typeface {
...
fun terminalTextInputUsesUtf8(
    output: String,
    hasPrefix: Boolean,
    hasRemoteInput: Boolean,
): Boolean = hasPrefix || hasRemoteInput || output.length != 1 || output.any { it.code > 0x7f }

fun terminalSetLinkHostAllowed(
    context: Context,
    host: String,
    allowed: Boolean,
) {
...
fun terminalOscHyperlinkAllowed(
    context: Context,
    value: String,
): Boolean {
