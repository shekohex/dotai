...
/**
 * Detect Obsidian vaults by reading Obsidian's config file Returns array of vault paths found on
 * the system
 */
...
export function extractTitle(markdown: string): string {
  ...
  if (h1Match) {
    ...
    return h1Match[1]
      .trim()
      .replaceAll(/[<>:"/\\|?*(){}[\]#~`]/g, "") // Remove invalid/problematic filename chars
 ...
 *
 * Supported variables: {title} - Plan title from first H1 heading {YYYY} - 4-digit year {MM} -
 * 2-digit month (01-12) {DD} - 2-digit day (01-31) {Mon} - Abbreviated month name (Jan, Feb, ...)
 * {D} - Day without leading zero {HH} - 2-digit hour, 24h (00-23) {h} - Hour without leading zero,
 * 12h {hh} - 2-digit hour, 12h (01-12) {mm} - 2-digit minutes (00-59) {ss} - 2-digit seconds
 * (00-59) {ampm} - am/pm
 *
 * Default format: '{title} - {Mon} {D}, {YYYY} {h}-{mm}{ampm}' Example output: 'User Authentication
 * - Jan 2, 2026 2-30pm.md'
 */
export function generateFilename(
  ...
  const result = template.replaceAll(/\{(\w+)\}/g, (match, key) => vars[key] ?? match);
...
/**
 * Generate YAML frontmatter for an Octarine note. Uses Octarine's property format (list-style tags,
 * Status, Author, Last Edited).
 */
