...
/**
 * Thrown when the agent loop ends with an API/stream failure (`stopReason` `"error"`/`"aborted"`)
 * without recording anything. agent-core returns such runs normally, so without this the caller
 * cannot tell a hard failure from a deliberate empty result (#32).
 */
...
export function normalizeSourceEntryIds(
  ...
  if (sourceEntryIds === undefined || sourceEntryIds.length === 0) return undefined;
...
/**
 * Build the record_observations tool bound to this run's accumulation state.
 *
 * @param {Map<string, Observation>} accumulated Observations recorded so far this run, by id.
 * @param {readonly string[]} allowedSourceEntryIds Source entry ids the model may cite.
 * @returns {AgentTool<typeof RecordObservationsSchema>} The record_observations tool.
 */
function makeRecordObservationsTool(
  ...
  return {
    ...
    execute: (_id, params: RecordObservationsArgs) => {
      let added = 0;
      let duplicates = 0;
      let rejected = 0;
...
export async function runObserver(args: RunObserverArgs): Promise<Observation[] | undefined> {
  ...
  const recordObservations = makeRecordObservationsTool(accumulated, allowedSourceEntryIds);
...
${conversation}`;
  ...
  let streamError: StreamFailure | undefined;
  for await (const event of stream) {
    ...
    const failure = streamFailureFromEvent(event);
    if (failure !== undefined) streamError = failure;
  }
  await stream.result();
