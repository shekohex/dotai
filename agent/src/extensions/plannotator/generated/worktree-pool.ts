 ...
 *
 * Runtime-agnostic. Uses ReviewGitRuntime for all git operations. Both Bun and Pi servers import
 * this module (Pi via vendor.sh).
 *
 * Each PR visited during a session gets its own worktree, created on first access and cached for
 * the session lifetime. Agents run in their PR's worktree undisturbed by PR switches.
 */

import { join } from "node:path";
import type { ReviewGitRuntime } from "./review-core.js";
import type { PRMetadata } from "./pr-provider.js";
import { createWorktree, removeWorktree, fetchRef, ensureObjectAvailable } from "./worktree.js";
