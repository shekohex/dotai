...
/**
 * Renders the bordered markdown preview block for a single question (one block per render call, for
 * the option at `optionIndex`). Owns a per-question `MarkdownContentCache`.
 *
 * NOT a `Component` — pure render-and-measure helper consumed by `PreviewPane`. The layout mode is
 * threaded as an explicit param (never re-derived from column width post-split).
 *
 * The affordance row is always emitted (visually empty when gated) so the preview block's row count
 * is height-stable across affordance-state transitions.
 */
export class PreviewBlockRenderer {
   ...
   * NOTES_AFFORDANCE_OVERHEAD`. Always returns the same value as `renderBlock(...).length` — the
   * affordance overhead is constant, not gated by `focused`/`notesVisible`.
   *
   * @param {number} width - Available width
   * @param {number} optionIndex - Option index
   * @param {PreviewLayoutMode} mode - Layout mode
   * @returns {number} Block height in rows
   */
   ...
   * `focused` and `notesVisible` together gate the affordance text (visible only when the focused
   * option carries a preview AND notes mode is inactive). The affordance row is ALWAYS emitted (as
   * an empty string when gated) so the row count is invariant.
   *
   * @param {number} width - Available width
   * @param {number} optionIndex - Option index
   * @param {PreviewLayoutMode} mode - Layout mode
   * @param {boolean} focused - Whether the pane is focused
   * @param {boolean} notesVisible - Whether notes are visible
  ...
  renderBlock(
    ...
    const affordance = showAffordance
      ? this.theme.fg("muted", uiText("preview.notes_affordance", NOTES_AFFORDANCE_TEXT))
      : "";
