 ...
 *
 * Saves plans and annotations to ~/.plannotator/plans/ Cross-platform: works on Windows, macOS, and
 * Linux.
 *
...
import { resolveUserPath } from "./resolve-file.js";

/**
 * Get the plan storage directory, creating it if needed. Cross-platform: uses os.homedir() for
...
/** Generate a slug from plan content. Format: {sanitized-heading}-YYYY-MM-DD */
...
/** Save the plan markdown to disk. Returns the full path to the saved file. */
...
/** Save annotations to disk. Returns the full path to the saved file. */
...
/**
 * Save the final snapshot on approve/deny. Combines plan and annotations into a single file with
 * status suffix. Returns the full path to the saved file.
 */
...
/**
 * Parse an archive filename into metadata. Handles both old (DATE-heading-status.md) and new
 * (heading-DATE-status.md) formats.
 */
...
/**
 * List all archived plans (approved/denied decision snapshots). Returns plans sorted by date
 * descending.
 */
export function listArchivedPlans(customPath?: string | null): ArchivedPlan[] {
  ...
  try {
    ...
    return plans.toSorted(
      (a, b) => b.date.localeCompare(a.date) || b.timestamp.localeCompare(a.timestamp),
    );
  } catch {
    return [];
  }
...
/** Read an archived plan file by filename. Returns null if the file doesn't exist or on read error. */
...
/**
 * Get the history directory for a project/slug combination, creating it if needed. History is
 * always stored in ~/.plannotator/history/{project}/{slug}/. Not affected by the customPath setting
 * (that only affects decision saves).
 */
...
/**
 * Determine the next version number by scanning existing files. Returns 1 if no versions exist,
 * otherwise max + 1.
 */
...
/**
 * Save a plan version to the history directory. Deduplication: if the latest version has identical
 * content, skip saving. Returns the version number, file path, and whether a new file was created.
 */
...
/**
 * Read a specific version's content from history. Returns null if the version doesn't exist or on
 * read error.
 */
...
/**
 * Get the file path for a specific version in history. Returns null if the version file doesn't
 * exist.
 */
...
/** Get the number of versions stored for a project/slug. Returns 0 if the directory doesn't exist. */
...
/**
 * List all versions for a project/slug with metadata. Returns versions sorted ascending by version
 * number.
 */
...
/** List all plan slugs stored for a project. Returns slugs sorted by most recently modified first. */
export function listProjectPlans(
  ...
  try {
    ...
    return plans.toSorted((a, b) => b.lastModified.localeCompare(a.lastModified));
  } catch {
    return [];
  }
}
