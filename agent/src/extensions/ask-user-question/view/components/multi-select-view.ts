...
/**
 * Renders the multi-select option list (one row per option — pointer + checkbox + label — plus zero
 * or more wrapped continuation lines per description).
 *
 * `naturalHeight(width)` is state-INDEPENDENT (depends only on theme glyph widths,
 * question.options, and width) so the host can compute a stable globalContentHeight without
 * rendering. `naturalHeight(w) === render(w).length` for every props.
 *
...
export class MultiSelectView implements StatefulView<MultiSelectViewProps> {
  ...
  render(width: number): string[] {
    ...
    for (let i = 0; i < this.question.options.length; i++) {
      ...
      if (opt === undefined || row === undefined) continue;
  ...
  /**
   * Returns the [startRow, endRow) range of the active (focused) row within `render(width)`. Labels
   * are always 1 row (truncated); descriptions wrap.
   *
  ...
  focusedItemRowRange(width: number): [number, number] {
    ...
    for (let i = 0; i < this.question.options.length; i++) {
      ...
      if (opt === undefined || r === undefined) continue;
      const itemHeight =
  ...
  naturalHeight(width: number): number {
    ...
    /* Next sentinel row (no description; never wraps). */
    return total + 1;
  }
