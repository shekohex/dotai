...
export function runStatusCommand(runtime: Runtime, ctx: ExtensionCommandContext): void {
  ...
  if (
    ...
    runtime.lastReflectorError !== undefined ||
    runtime.lastDropperError !== undefined
  ) {
    lines.push("", "── Last error ──");
    if (runtime.lastObserverError !== undefined)
      lines.push(`Observer: ${runtime.lastObserverError}`);
    if (runtime.lastReflectorError !== undefined)
      lines.push(`Reflector: ${runtime.lastReflectorError}`);
    if (runtime.lastDropperError !== undefined) lines.push(`Dropper: ${runtime.lastDropperError}`);
  }
