 ...
 *
 * Pi consumes a build-time copy of this file so its published package stays self-contained while
 * review diff logic remains sourced from one module.
 */
...
/**
 * Query the remote for its default branch via `ls-remote --symref`. Returns `origin/<name>` if the
 * remote answers and the tracking ref exists locally, otherwise `null`. Designed to run in the
 * background at server startup — the caller fires it with `.then()` and uses the result if/when it
 * arrives.
 *
 * Timeout-guarded: if the network is slow or absent, the promise resolves (with `null`) once the
 * timeout fires. Never throws.
 */
...
/**
 * Pick a safe base branch. Trusts the caller verbatim if they supplied one, otherwise falls back to
 * the detected default. Shared by Bun (`review.ts`) and Pi (`serverReview.ts`) so both runtimes
 * behave identically.
 *
 * Why trust the caller: the UI picker only ever sends refs from the known list, and
 * external/programmatic callers may pass tags, SHAs, or refs under non-`origin` remotes that we
 * must not silently rewrite (a tag `release` is not the same commit as a branch `origin/release`).
 * Invalid refs surface as git errors on the next diff call, which is better than silently producing
