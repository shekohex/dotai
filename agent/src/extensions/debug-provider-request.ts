...
function extractSystemPrompt(payload: unknown): string | undefined {
	const candidateKeys = new Set(["systemPrompt", "systemInstruction", "instructions", "instruction", "system"]);
...
export default function debugProviderRequestExtension(pi: ExtensionAPI) {
	const enabled = isTruthy(process.env.PI_DEBUG_PROVIDER_REQUESTS) || isTruthy(process.env.PI_DEBUG_SYSTEM_PROMPT);
	if (!enabled) {
		return;
	}
