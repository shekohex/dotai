 ...
 *
 * Reads a GSD-2 project directory structure and produces a complete .planning/ artifact tree in GSD
 * v1 format.
 *
 * GSD-2 hierarchy: Milestone → Slice → Task GSD v1 hierarchy: Milestone (in ROADMAP.md) → Phase →
 * Plan
 *
 * Mapping rules: - Slices are numbered sequentially across all milestones (01, 02, …) - Tasks
 * within a slice become plans (01-01, 01-02, …) - Completed slices ([x] in ROADMAP) → [x] phases in
 * ROADMAP.md - Tasks with a SUMMARY file → SUMMARY.md written - Slice RESEARCH.md → phase
 * XX-RESEARCH.md
 */
...
/**
 * Find the .gsd/ directory starting from a project root. Returns the absolute path or null if not
 * found.
 */
...
/**
 * Parse the ## Slices section from a GSD-2 milestone ROADMAP.md. Each slice entry looks like:
 *
 * - **S01: Title** `risk:medium` `depends:[S00]`
 */
...
/** Parse the milestone title from the first heading in a GSD-2 ROADMAP.md. Format: # M001: Title */
function parseMilestoneTitle(content) {
  const m = content.match(/^# \w+:\s*(.+)/m);
  return m ? m[1].trim() : null;
}

/** Parse a task title from a GSD-2 T##-PLAN.md. Format: # T01: Title */
...
function parseGsd2(gsdDir) {
  ...
  const milestoneIds = fs
    .readdirSync(milestonesBase)
...
/**
 * Build a GSD v1 SUMMARY.md from a GSD-2 task summary. Strips the GSD-2 frontmatter and preserves
 * the body.
 */
...
/**
 * Convert parsed GSD-2 data into a map of relative path → file content. All paths are relative to
 * the .planning/ root.
 */
...
function buildPreview(gsd2Data, artifacts) {
  ...
  const doneSlices = gsd2Data.milestones.reduce(
...
/** Entry point called from gsd-tools.cjs. Supports: --force, --dry-run, --path <dir> */
function cmdFromGsd2(args, cwd, raw) {
  ...
  const projectDir = pathIdx >= 0 && args[pathIdx + 1] ? path.resolve(cwd, args[pathIdx + 1]) : cwd;
