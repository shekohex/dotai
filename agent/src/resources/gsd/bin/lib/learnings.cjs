 ...
 *
 * Provides a cross-project learnings store at ~/.gsd/knowledge/. Each learning is stored as an
 * individual JSON file with content-hash deduplication. Supports write, read, list, query, delete,
 * copy-from-project, and prune operations.
 *
 * Storage format: { id, source_project, date, context, learning, tags, content_hash } File naming:
 * {id}.json Deduplication: SHA-256 of learning text + source_project
 */
...
/**
 * Generate a content hash for deduplication. Uses SHA-256 of learning text combined with
 * source_project.
 *
...
/**
 * Read and parse a single learning JSON file. Returns null (with stderr warning) for malformed
 * files.
 *
...
/**
 * Write a learning to the global store. Deduplicates by content hash — same content from same
 * project is not stored twice.
 *
 * @param {object} entry
...
/**
 * Copy learnings from a project's LEARNINGS.md into the global store. Parses markdown sections as
 * individual learnings. Deduplicates by content hash.
 *
