...
2. Run `scripts/coordinator_state.py summary --product-id <id>`. Initialize only when state does not exist. Read-only commands validate ordered schema history without writing. Unknown or newer schema versions refuse safely.
...
5. Acquire product coordinator lease before mutation.
