...
test("gsd help content mentions enablement gate and local-only guardrails", async () => {
  ...
  expect(content).toContain(
    "config-disabled Nyquist validation, ambiguous/non-canonical VALIDATION inventory, malformed helper payloads, helper execution failures, and non-canonical helper target paths fail closed before workflow launch",
  );
  expect(content).toContain(
