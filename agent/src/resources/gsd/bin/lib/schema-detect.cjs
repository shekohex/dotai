/**
 * Schema Drift Detection — Detects schema-relevant file changes and verifies that the appropriate
 * database push command was executed during a phase.
 *
 * Prevents false-positive verification when schema files change but no push occurs — TypeScript
 * types come from config, not the live database, so build/types pass on a broken state.
 */
