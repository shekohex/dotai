...
/**
 * User-facing labels for the three runtime sentinel rows, keyed by their `WrappingSelectItem.kind`
 * discriminator. Sourced from `ROW_INTENT_META` via `LABELS_BY_KIND` (`row-intent.ts`) — single
 * source of truth. Adding a new sentinel requires extending the `WrappingSelectItem` union AND
 * adding an entry to `ROW_INTENT_META`; this map then auto-extends.
 */
...
/**
 * Labels reserved for Pi-internal sentinels — authoring an option with any of these labels triggers
 * the `reserved_label` runtime guard. Three of the four come from `ROW_INTENT_META` (the runtime
 * kinds); `"Other"` is reserved for CC parity only (the model is conditioned to reach for "Other"
 * in CC; we reject it so the runtime sentinel is the single source of truth) and has no runtime
 * kind.
 *
 * Reserved unconditionally — multiSelect questions also reject these labels even though the runtime
 * sentinel is suppressed there.
 *
 * Order is pinned by `types.test.ts:292` — keep the explicit `["Other", other, chat, next]` literal
 * so consumers using `RESERVED_LABELS[i]` indexing or `Set` membership see no behavior change.
 */
...
/**
 * Answer-intent discriminated union. `kind` is the single discriminator — pre-1.0.3 boolean flags
 * have been removed (see `banned-flags.test.ts`). Mirrors the row-side `WrappingSelectItem.kind`
 * vocabulary where possible; `multi` is the multi-select variant (no row-side analog).
 *
 * Variant semantics: - `option`: user picked one of the author-defined options. `answer` is the
 * option's label. - `custom`: user typed free-text via the "Type something." row. `answer` is the
 * typed text or null. - `chat`: user picked the chat sentinel. `answer` is the literal "Chat about
 * this". - `multi`: user committed multi-select choices. `selected` carries chosen labels; `answer`
 * is null.
 */
export interface QuestionAnswer {
  ...
  notes?: string;
  /**
   * Markdown text from the matched option's `preview` field, populated only when the user lands on
   * a single-select option carrying a `preview`. Used by `buildQuestionnaireResponse` to echo
   * `selected preview: <preview>` into the LLM-facing envelope. Undefined for multi-select,
   * custom-text (`kind: "custom"`), and chat (`kind: "chat"`) answers.
   */
  preview?: string;
}
...
export function isQuestionnaireResult(value: unknown): value is QuestionnaireResult {
  ...
  if (!("answers" in value) || !("cancelled" in value)) return false;
