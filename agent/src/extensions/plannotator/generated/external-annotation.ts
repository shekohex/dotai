 ...
 *
 * Runtime-agnostic: no node:fs, no node:http, no Bun APIs. Both the Bun server handler and Pi
 * server handler import this module and wrap it with their respective HTTP transport layers.
 *
 * The store is generic — plan servers store Annotation objects, review servers store CodeAnnotation
 * objects. The mode-specific input transformers handle validation and field assignment.
 */
 ...
 *
 * Accepts either: - A single annotation object: `{ source: "...", ... }` - A batch wrapper: `{
 * annotations: [{ source: "...", ... }, ...] }`
 */
...
export function transformPlanInput(body: unknown): { annotations: PlanAnnotation[] } | ParseError {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (
      type === "DELETION" &&
      (typeof obj.originalText !== "string" || obj.originalText.length === 0)
    ) {
 ...
 *
 * The store is runtime-agnostic — it holds data and emits events. HTTP transport (SSE broadcasting,
 * request parsing) is handled by the server-specific adapter (Bun or Pi).
 */
