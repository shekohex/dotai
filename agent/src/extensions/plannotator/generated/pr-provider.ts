 ...
 *
 * Dispatches to platform-specific implementations (GitHub, GitLab) based on the `platform` field in
 * PRRef/PRMetadata.
 *
 * Same pattern as review-core.ts: a runtime interface abstracts subprocess execution so the logic
 * is reusable across Bun and Node/jiti.
 */
...
export interface GithubPRMetadata {
  ...
  headSha: string;
  /**
   * Merge-base SHA — the common ancestor commit used to compute the PR diff. Differs from baseSha
 ...
 *
 * GitLab is checked first because `/-/merge_requests/` is unambiguous, while `/pull/` could
 * theoretically appear on any host.
 */
...
/**
 * Fetch per-file "viewed" state for a PR. GitHub: returns { filePath: isViewed } map. GitLab:
 * always returns {} (no server-side viewed state API).
 */
...
/**
 * Mark or unmark files as viewed in a PR. GitHub: fires markFileAsViewed / unmarkFileAsViewed
 * GraphQL mutations. GitLab: no-op (no server-side viewed state API).
 */
...
/**
 * Fetch the full stack tree for a stacked PR. Walks up from the current PR to the default branch,
 * resolving PR numbers and titles for each intermediate branch. Returns null if the PR is not
 * stacked or the API call fails.
 */
