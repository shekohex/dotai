...
import { displayLabel, uiText } from "./labels.js";
...
/**
 * Slim runtime: owns the canonical state cell, the input-buffer cell, the two-pass `notesVisible`
 * dispatch loop, and the effect runner. State transitions go through the pure `reduce` reducer; UI
 * fan-out goes through the `QuestionnairePropsAdapter` produced by `buildQuestionnaire`.
 */
export class QuestionnaireSession {
  ...
  constructor(config: QuestionnaireSessionConfig) {
    ...
    const collapsedRender = (_width: number): string[] => [
      theme.fg("dim", ` ${uiText("hint.expand_line", COLLAPSED_HINT)} `),
    ];
  ...
  /**
   * Sync notes draft from the notes Input back into state.
   *
   * @param {QuestionnaireState} s - Current state.
   * @returns {QuestionnaireState} Updated state with fresh notesDraft.
   */
  ...
  /**
   * Execute a declarative side-effect.
   *
  ...
  /**
   * Per-keystroke `ignore` fast path: delegates to the headless `inlineInput` Input so
   * bracketed-paste accumulator (`input.js:33-63`) and Kitty CSI-u decode (`input.js:155-163`) take
   * effect. Cursor is NOT force-reset here — doing so would corrupt split-chunk pastes (a `\x05`
   * byte mid-paste lands verbatim in `pasteBuffer` and survives `handlePaste`'s narrow strip).
   * Cursor advances naturally via `insertCharacter` on typing/paste; cursor- movement keys
   * (Left/Right/Home/End/word-jumps) are now functional, with the always-end visual cursor marker
   * drawn independently by `WrappingSelect.renderInlineInputRow`. `viewAdapter.apply` is called
   * directly without a reducer round-trip — preserves the D3 fast-path latency profile from Phase
   * 11.
   *
