...
Status: review, target-device proof captured.
...
- Records speech and detects speech end: `SpeechAudioCapture.kt` implements `AudioRecord`, metering, VAD, silence handling, and cleanup; JVM speech tests pass; Pixel 7 Pro Mac TTS live-mic proof captured under `build/validation/speech-real-device/agent-device-tts/final-rerun/`.
...
- Insert/send through existing chat draft behavior: `mergeSpeechTranscriptIntoDraft` and `CoderApp.kt` route accepted speech through `chatDraft`; `SpeechChatDraftTest` passes; `build/validation/speech-real-device/agent-device-tts/final-rerun/after-accept.png` shows accepted Pixel transcript in editable chat draft.
...
- Local Parakeet runs through LiteRT on Pixel 7 Pro or Samsung Tab A9: Pixel 7 Pro smoke helper restored model/tokenizer and passed device UI instrumentation under `build/validation/speech-real-device/20260521-183411/`; manual Mac TTS live-mic proof produced final local transcript under `build/validation/speech-real-device/agent-device-tts/final-rerun/retry-after-finish-65s.png`.
- Real-device support proof: Pixel 7 Pro attached as `192.168.1.200:41563`; latest app build accepted local transcript into draft under `build/validation/speech-real-device/agent-device-tts/final-rerun/after-accept.png`.
...
- Manual live mic Parakeet attempt `build/validation/speech-real-device/manual-20260521-183929/`: mic permission granted, chat dictation UI opened, waveform rendered, AudioRecord/LiteRT loaded on Pixel, no fatal crash markers appeared, but finish produced a blank transcript-ready state. Fixed follow-up: blank final transcription now enters `NO_SPEECH` retry/cancel state instead of any sendable transcript-ready state, and blank final output falls back to live partial transcript only when non-blank.
- Latest Pixel 7 Pro Mac TTS proof after review fixes: installed latest debug APK on `192.168.1.200:41563`, opened chat dictation with `agent-device`, played `say -v Samantha 'open settings and explain the failing gradle task. open settings and explain the failing gradle task.'` at output volume 90, final local transcript was visible as `Open settings and explain the failing gradle test.` in `build/validation/speech-real-device/agent-device-tts/final-rerun/retry-after-finish-65s.png`, and accepting it inserted the text into editable chat draft without submitting in `build/validation/speech-real-device/agent-device-tts/final-rerun/after-accept.png`.
- Device-control learning: prefer `agent-device` over raw `adb` for interactive Pixel/emulator UI control (`agent-device help workflow`). On macOS, use `say`/`afplay` to play a spoken fixture through Mac speakers when Pixel 7 Pro is physically nearby, then capture screenshots/logs to prove mic/VAD/LiteRT behavior.
...
- UI polish follow-up: Pixel 7 Pro debug screenshots showed the live transcript surface wasting width, final transcript states hiding transcript text after compacting the VoiceInk-style bubble, secondary text actions being too small inside the recorder bubble, the red primary action overpowering the card, and duplicate send affordances caused by UI deriving actions from loose capability flags. Dictation surface now centers a bounded recorder bubble, keeps waveform/action aligned, restores transcript text for final/enhanced/failure states, moves secondary actions outside as larger 44dp Feather icon chips, uses a 40dp primary glyph inside a 48dp hit target, adds primary action accessibility labels, and gets all visible primary/secondary actions from `SpeechDictationUxContract.visibleActionsFor`. Unit tests verify visible actions are unique and valid for each state. Emulator UI Automator journey passed for the latest state-machine UX; screenshots were captured under `build/validation/speech-ui/final-fix/`, `build/validation/speech-ui/chips/`, and `build/validation/speech-ui/state-machine/`.
- VAD/live transcription review: compared against Google LiteRT `MicrophoneAudioSource.kt`, which zeroes samples below `SILENCE_THRESHOLD` but keeps yielding fixed overlapping windows, and VoiceInk streaming lifecycle, which keeps recording active until explicit stop while partial text updates continue only from meaningful audio. Android docs search did not show a standalone built-in VAD API for raw `AudioRecord`; GitHub examples either use WebRTC VAD native libraries or RMS+peak frame gates. Because native linkage changes require approval, current implementation stays pure Kotlin and now uses RMS plus peak thresholds, exposes `voiceActive` and `speechPaused`, zeroes below-threshold silent frames before buffering, never finalizes automatically on trailing silence, and pauses partial transcription scheduling after configured trailing silence until voice resumes.
- VoiceInk performance follow-up: reviewed `/Users/shady/github/Beingpax/VoiceInk/VoiceInk/Services/ModelPrewarmService.swift`, `Transcription/Streaming/StreamingTranscriptionService.swift`, `Transcription/Streaming/FluidAudioStreamingProvider.swift`, and `Transcription/Streaming/WordAgreementEngine.swift`. Android now exposes model prewarm settings, cycles warm intervals between 5/10/15/30/60 minutes, records local-only timing metrics (`total`, warmup, feature extraction, encode, decode, tokenizer, first token, token count, samples, sample rate, accelerator, warm/cold), hides waveform in ready states, and moves partial scheduling closer to VoiceInk's streaming cadence with 1s checks, up to 15s rolling context, and 1s trailing silence. Current LiteRT dependency is `com.google.ai.edge.litert:litert:2.1.5`; `javap` verified `Accelerator` supports `NONE`, `CPU`, `GPU`, and `NPU`. Settings expose `Auto`, `CPU`, `GPU`, `NPU`, and `None`; `Auto` passes NPU/GPU/CPU to LiteRT.
- Warm-cache correction: keep-model-warm is now enabled by default for this personal app. `SpeechWarmModelService` owns a process-level `SpeechWarmModelStore` and keeps the shared `LiteRtParakeetTranscriber` alive from a foreground service, including while the app is backgrounded. The shared transcriber retains the LiteRT `CompiledModel`, encode/decode tensor buffers, TDT decoder buffers, and tokenizer until the user disables warm mode, model/accelerator settings change, or the process dies. This removes compile/tokenizer setup from tap-to-first-token when model/tokenizer are ready; actual GPU/NPU residency is determined by LiteRT delegate registration and Pixel logs still need to confirm which accelerator was accepted.
- Warm-cache release validation: release build installed with `adb install -r` on Pixel 7 Pro so app data/model cache is preserved. `SpeechWarmModelService` starts from app root, remains foreground while app can background, auto-imports side-loaded model/tokenizer before warming, and Pixel release logs show LiteRT NPU registration rejected but GPU accelerator registered and TFLite delegated all encode/decode subgraphs with `LITERT_CL`; warm completed with `speech_metrics total_ms=6025 warmup_ms=6025 accelerator=Auto model_warm=false` on first load.
- VoiceInk sound feedback follow-up: copied VoiceInk `recstart.mp3`, `recstop.mp3`, and `esc.wav` into Android raw resources, defaulted sound feedback on, added a Voice Input `Sound Feedback` toggle, and wired start/stop/cancel/failure feedback using VoiceInk-style volumes (`0.4` for start/stop, `0.3` for esc/failure).
- VoiceInk processing UI follow-up: `TRANSCRIBING` and `ENHANCING_COLLAPSED` now collapse the dictation surface into the compact recorder pill, hide transcript text while processing, and show a centered VoiceInk-style processing label plus five-dot progress animation. Transcript text expands again when final/enhanced text is ready.
- VoiceInk recorder polish follow-up: dictation surface now uses VoiceInk mini recorder proportions (`184dp` compact, `300dp` expanded, `40dp` control bar, `20dp`/`14dp` corner radii), keeps live transcript above the control bar with divider, places enhancement action inline on the left, centers waveform/processing/static bars, keeps the primary action as a small right-side circular control, and preserves app theme colors instead of copying VoiceInk black/white styling.
- Enhancement provider follow-up: Speech settings now expose `Enhance Transcript`, provider selection (`OpenAI Compatible`, `Gemini`, `Disabled`), encrypted API key entry, model entry, OpenAI-compatible endpoint entry, prompt editing, and visible-terminal-context toggle. `TerminalAccessory` builds a real `SpeechEnhancementClient` from settings and passes it into `ChatInputBar`; enhancement requests use bounded/redacted transcript/context and fail open to raw transcript.
- Warm/shared-transcriber review fix: shared `LiteRtParakeetTranscriber` now serializes `warm`, `transcribe`, and `close` with an internal lock so foreground service warm/close cannot mutate LiteRT buffers while UI partial/final transcription is running. If the UI stops recording before the shared transcriber is assigned, it now shows a visible failure instead of staying stuck in `TRANSCRIBING`. Final chunking uses 2s overlap for stronger text merge at slow boundary speech.
- Latest release validation: `adb install -r app/build/outputs/apk/release/app-arm64-v8a-release.apk` succeeded on Pixel 7 Pro and preserved app data/cache. Opening `pi://settings/speech` started `SpeechWarmModelService`; `dumpsys activity services com.coder.pi` showed `isForeground=true`, and logcat showed LiteRT GPU registered, NPU rejected, and encoder delegated with `LITERT_CL` without `FATAL`/`AndroidRuntime` errors in the captured excerpt.
- Enhancement settings release validation: `./gradlew testDebugUnitTest --tests '*Speech*' --no-daemon` and `./gradlew assembleRelease --no-daemon` passed. Release APK was updated with `adb install -r`, Speech settings opened successfully on Pixel 7 Pro, screenshot captured at `build/validation/speech-release/enhancement-settings.png`, and warm service remained foreground.
- Enhancement secret review fixes: review `496d53ad-44b9-42b5-9095-9057a7e90e75` found plaintext API-key prefill, stale enhancement client after key rotation, and missing `speech_secure.xml` backup excludes. Fixed by never pre-filling the key field, reading latest provider/key/model at enhancement call time, and excluding `speech_secure.xml` from backup/data extraction. Re-review `0c80f54e-8d28-4adb-9a65-128b012d9270` found missing clear path and skipped enhancement being classified as enhanced. Fixed by making blank save clear the stored key and returning an empty skipped result so `SpeechEnhancer` fails open to raw transcript with `enhanced=false`.
- Enhancement secret validation after fixes: `./gradlew testDebugUnitTest --tests '*Speech*' --no-daemon` passed and `./gradlew assembleRelease --no-daemon` passed after `9adcd60`.
- Recorder polish validation: `./gradlew testDebugUnitTest --tests '*Speech*' --no-daemon` passed and `./gradlew assembleDebug --no-daemon` passed after VoiceInk mini recorder polish.
- Performance validation: `./gradlew testDebugUnitTest --tests '*Speech*' --tests '*LiveSpeech*' --no-daemon` passed after warm/metrics/accelerator changes, and `./gradlew assembleDebug assembleDebugAndroidTest --no-daemon` passed. Metrics log line is bounded to timing/counts/accelerator only and does not include transcript, prompt, context, audio, request/response bodies, URLs, or secrets.
- Pixel performance optimization follow-up: current installed benchmark APK was debug because release install was blocked by `INSTALL_FAILED_UPDATE_INCOMPATIBLE` signature mismatch with the existing debug install. Naive Kotlin DFT feature extraction measured about `8181-8736ms` per 15s live chunk before optimization. Kotlin radix-2 FFT reduced hot feature extraction to about `163-224ms`; adding precomputed FFT twiddles, sparse Mel filters, cached TDT decoder buffers, and cached tokenizer reduced hot feature extraction to about `27-94ms` and tokenizer to `0-1ms` under `build/validation/speech-real-device/agent-device-tts/feature-opt2/sanitized-log.txt`. Remaining hot-path bottleneck is TDT decode, about `802-1827ms`; Google sample `TdtDecoder.kt` is Kotlin and structurally matches this repeated `decode`/`decode_1` LiteRT loop. Pixel 7 Pro logs show NPU registration fails with `kLiteRtStatusErrorInvalidArgument`, while LiteRT GPU registers successfully.
- VoiceInk streaming-loop hardening: replaced per-frame partial scheduling with a single session-owned loop modeled after VoiceInk `FluidAudioStreamingProvider.startTranscriptionLoop`: the audio callback only appends frames and updates metering, the loop sleeps for 1s, runs at most one pass, awaits completion, and exits on session/state change. This prevents older frame-triggered jobs from applying transcript text after newer speech. Dictation waveform now uses the latest 15 audio meter samples instead of a fake infinite sine animation, matching VoiceInk's audio-meter-driven visualizer direction.
- Decode optimization follow-up: model is the 5s LiteRT Parakeet export, so live partial snapshots now use the latest 4s window plus 1s trailing silence, preserving first pass after 1s while reducing subsequent pass threshold to 0.5s new audio. Decoder now receives a valid-frame limit derived from live input length and logs `decode_steps` / `decode_frame_limit`; Pixel proof under `build/validation/speech-real-device/agent-device-tts/decode-opt1/sanitized-log.txt` showed `samples=40000`, `decode_steps=15`, `decode_frame_limit=31` before this VoiceInk-alignment pass. LiteRT docs and Google AI Edge Gallery references were checked: the Acceleration Service is Play Services-only and CPU/GPU-only, while Gallery declares optional native libraries `libOpenCL.so`, `libcdsprpc.so`, and `libedgetpu_litert.so`; the Android manifest now mirrors those optional hardware libraries for bundled LiteRT delegate discovery.
- VoiceInk alignment validation: `./gradlew testDebugUnitTest --tests '*Speech*' --no-daemon` passed after adding coverage for 4s live window, 1s trailing silence, 0.5s pass advancement, long-final live transcript preservation, and early-window pre-padding. `./gradlew assembleDebug --no-daemon` passed.
...
- Release side-load support: `54ba0e2` (`feat(android): support release speech model sideload`).
- Core VoiceInk audio gaps: `a38c91f` (`fix(android): close core VoiceInk audio gaps`).
- Speech enhancement actions/preflight: `1280fe9` (`fix(android): wire speech enhancement actions`).
...
- Enhancement secret hardening: `1c76057` (`fix(android): harden speech enhancement secrets`) and `9adcd60` (`fix(android): clear stale speech enhancement keys`).
