...
The connected surface collapses into a compact bottom-center glass strip with a continuously animated orb, layered Siri-style waveform, and audio-reactive Apple Intelligence glow border adapted from `jacobamobin/AppleIntelligenceGlowEffect`. Space toggles mute while the call window is focused. Muting hides the microphone waveform, but remote output still drives the orb and glow so Pi remains visibly alive while speaking. Assistant settings include custom conversational instructions; pairing persists them to `live.instructions`, while changes during a call apply to the next call. `src/resources/live/live-instructions.md` is the single canonical prompt. It forbids transcript-by-transcript delegation, requires synthesized English AgentSession requests, and keeps spoken replies in the user's source language.
