 ...
 *
 * Reads REQUIREMENTS.md (planning-root) and CONTEXT.md (per-phase) and compares each REQ-ID and
 * D-ID against the concatenated text of all PLAN.md files in the phase directory. Emits a unified
 * `Source | Item | Status` report.
 *
 * Gated on workflow.post_planning_gaps (default true). When false, returns { enabled: false } and
 * does not scan.
 *
 * Coverage detection uses word-boundary regex matching to avoid false positives (REQ-1 must not
 * match REQ-10).
 */
 ...
 *
 * Supports both checkbox (`- [ ] **REQ-NN** ...`) and traceability table (`| REQ-NN | ... |`)
 * formats.
 */
