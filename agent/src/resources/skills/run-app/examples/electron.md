...
at `.pi/skills/run-desktop/`; the driver finds a better home.
...
tmux send-keys -t app 'cd /workspace/apps/desktop && xvfb-run -a node .pi/skills/run-desktop/driver.mjs' Enter
...
> via the Playwright REPL at `.pi/skills/run-desktop/driver.mjs`
...
> ```bash
> cd apps/desktop
> xvfb-run -a node .pi/skills/run-desktop/driver.mjs
> ```
>
...
> ```bash
> tmux new-session -d -s app -x 200 -y 50
> tmux send-keys -t app 'cd apps/desktop && xvfb-run -a node .pi/skills/run-desktop/driver.mjs' Enter
> timeout 20 bash -c 'until tmux capture-pane -t app -p | grep -q "driver>"; do sleep 0.2; done'
> tmux send-keys -t app 'launch' Enter
> timeout 60 bash -c 'until tmux capture-pane -t app -p | grep -q "launched"; do sleep 0.2; done'
