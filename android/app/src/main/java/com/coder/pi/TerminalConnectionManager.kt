...
object TerminalConnectionManager {
    ...
    fun startHeadless(
        ...
        notificationContext: TerminalNotificationContext,
        ...
        val shouldStart =
            synchronized(sessions) {
                if (sessions.containsKey(notificationContext.terminalId)) {
                    false
                } else {
    ...
    fun detachRenderer(terminalId: String) {
        ...
        val visibleEndpoint =
            runtime.endpoint as? CoderTerminalView ?: run {
    ...
    private fun attachRendererToExistingSession(
        terminalId: String,
        endpoint: CoderTerminalEndpoint,
    ): CoderTerminalSession? {
        if (endpoint is CoderTerminalView) endpoint.releaseEngineOwnershipToManager()
    ...
    private data class RuntimeSession(
        val proxy: TerminalEndpointProxy,
        val endpoint: CoderTerminalEndpoint,
