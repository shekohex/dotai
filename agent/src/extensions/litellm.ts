...
const CODEX_OPENAI_EXTENDED_CONTEXT_MODEL_IDS = new Set([
  "gpt-5.6-luna",
  "gpt-5.6-sol",
  "gpt-5.6-terra",
]);
const CODEX_OPENAI_EXTENDED_CONTEXT_WINDOW = 258_000;
...
function createCodexOpenAIModels(): ProviderModelConfig[] {
  return getBuiltinModels("openai-codex").map((model) => ({
    ...
    input: [...model.input],
    cost: { ...model.cost },
    contextWindow: CODEX_OPENAI_EXTENDED_CONTEXT_MODEL_IDS.has(model.id)
      ? CODEX_OPENAI_EXTENDED_CONTEXT_WINDOW
      : model.contextWindow,
    maxTokens: model.maxTokens,
    compat: model.compat,
    headers: model.headers,
