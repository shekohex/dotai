...
- `src/resources/prompts/gsd/...`
- `src/resources/modes/gsd/...`
- `src/resources/docs/gsd/...` or bundled docs path
- optional `src/resources/templates/gsd/...` if useful
...
export const StateFrontmatterSchema = Type.Object(
  {
    gsd_state_version: Type.Optional(Type.String()),
    milestone: Type.Optional(Type.String()),
    milestone_name: Type.Optional(Type.String()),
    current_phase: Type.Optional(Type.String()),
...
export const PlanFrontmatterSchema = Type.Object(
  {
    ...
    must_haves: Type.Union([Type.String(), Type.Array(Type.String())]),
  },
  { additionalProperties: true },
);
...
const ROLE_REGISTRY: Record<GsdRole, GsdRoleConfig> = {
  planner: {
    ...
    bundledPromptPath: "src/resources/prompts/gsd/planner.md",
    defaultSpec: {
  ...
  executor: {
    ...
    bundledPromptPath: "src/resources/prompts/gsd/executor.md",
    defaultSpec: {
  ...
  verifier: {
    ...
    bundledPromptPath: "src/resources/prompts/gsd/verifier.md",
    defaultSpec: {
  ...
  "codebase-mapper": {
    ...
    bundledPromptPath: "src/resources/prompts/gsd/codebase-mapper.md",
    defaultSpec: {
...
Bundled resources are discovered via the existing `bundled-resources.ts` patch. GSD-specific prompts and modes live under `src/resources/prompts/gsd/` and `src/resources/modes/gsd/`.
...
Example bundled prompt (`src/resources/prompts/gsd/executor.md`):
...
1. **Mode injection vs. user-managed modes**
   Should the GSD extension auto-inject bundled `gsd-*` mode definitions into the mode system at session start, or should it document that users must add them to their global modes file? Auto-injection is friendlier but may conflict with user customizations.

2. **Phase directory naming conventions**
   Upstream GSD uses `01-setup/`, `02-core/`, etc. `pi-gsd` also supports dotted phases (`3.1`). Should our brownfield detection accept both, or normalize to one style on write?

3. **Structured output retry budget defaults**
   The subagent SDK defaults to 3 retries for `json_schema` output. Should GSD commands override this per-role (e.g., planner gets 3, executor gets 1 because it runs longer), or keep a uniform default?

4. **Enablement persistence scope**
   Should `/gsd on` be persisted per-workspace (e.g., in `.pi/state` or a dotfile), or per-session only? Per-workspace is more convenient but requires a persistence mechanism we do not yet use for other toggles.

5. **ROADMAP.md parsing strategy**
   ROADMAP.md is free-form markdown. Do we port the regex-heavy parser from `pi-gsd` (`~/.cache/checkouts/github.com/fulgidus/pi-gsd/src/lib/roadmap.ts`), or treat it as loosely structured and derive phase list from the `phases/` directory instead?

6. **Subagent SDK gaps for orchestration scale**
   If a phase has 6 plans and parallelization is enabled, we may spawn up to 3 concurrent executors. Does the current subagent SDK handle concurrent spawn throttling, or should the GSD extension implement a semaphore?

7. **Testing strategy for lifecycle commands**
   Lifecycle commands (`plan-phase`, `execute-phase`) may run for minutes and spawn real subagents. Should we invest in mocked subagent SDK tests, or rely on integration tests against a fixture repo?

8. **Resource bundling for mode definitions**
   Mode definitions are currently loaded from the project or global modes file. If we bundle `gsd-*` mode markdown under `src/resources/modes/gsd/`, how do we register them so the mode system can resolve them? The bundled-resources patch only handles skills, prompts, and themes today.
