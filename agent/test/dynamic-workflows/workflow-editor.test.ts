...
test("installWorkflowInputHooks restricts tools to `workflow` while armed, then restores", () => {
  ...
  assert.deepEqual(
    activeTools,
    ["workflow"],
    "only the workflow tool is active during the forced turn",
  );
  ...
  assert.deepEqual(
    activeTools,
    ["read", "bash", "workflow", "subagent"],
    "tools restored after the turn",
  );
...
test("installWorkflowInputHooks does not restrict tools after conversation started", () => {
  ...
  state.active = true;
  assert.deepEqual(handlers.input({ source: "interactive", text: "use workflow here" }, ctx), {
    action: "continue",
  });
...
test("installWorkflowInputHooks activates deferred workflow before restricting the turn", () => {
  const handlers: Record<string, (event: any, ctx?: any) => any> = {};
  ...
  installWorkflowInputHooks(pi, state, {
    activateWorkflowTool() {
      activationCount += 1;
      activeTools = [...activeTools, "workflow"];
    },
  });

  state.active = true;
  const result = handlers.input({ source: "interactive", text: "use workflow here" });

  assert.equal(result.action, "transform");
  assert.equal(activationCount, 1);
  assert.equal(state.active, false, "arm is consumed after transforming");
  assert.deepEqual(activeTools, ["workflow"]);
});
