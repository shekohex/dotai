...
- **Defaults** — `src/default-settings.ts` exports `defaultSettings` (typed as `DefaultSettings`), composed from per-extension default settings (e.g. `defaultOpenAIBetterSettings`, `defaultContextPruneSettings`, `defaultDynamicWorkflowSettings`, `defaultAiAutocompleteSettings`, ...). Key top-level defaults: `defaultProvider: "openai-codex"`, `defaultModel: "gpt-5.6-sol"`, `theme: "catppuccin-latte/catppuccin-mocha"` (light/dark pair), `hideThinkingBlock: true`, `retry.maxRetries: 1024`.
