...
import { isCustomEntry, isPruneFrontierData, type PruneFrontierData } from "./guards.js";

/**
 * Tracks the most recent completed prune-attempt boundary.
 *
 * The frontier advances when a prune attempt finishes, regardless of whether it produced a
 * persisted summary or was skipped because the summary was larger than the raw tool outputs. It
 * does not advance on operational failures.
 */
export class PruneFrontierTracker {
  ...
  fromJSON(data: PruneFrontierData): void {
    this.frontier = {
  ...
  reconstructFromSession(ctx: ExtensionContext): void {
    ...
    for (const entry of branch) {
      if (isCustomEntry(entry, CUSTOM_TYPE_FRONTIER, isPruneFrontierData)) {
