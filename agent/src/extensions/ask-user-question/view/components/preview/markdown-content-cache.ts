...
/**
 * 1 blank separator + 1 affordance text row reserved when `hasAnyPreview` (height stability of the
 * affordance row's offset relative to the box).
 */
export const NOTES_AFFORDANCE_OVERHEAD = 2;

/**
 * Per-question cache for rendered markdown previews. Width-keyed: switching the inner width
 * invalidates every cached `Markdown`'s render output (pi-tui's `Markdown.render(width)` re-wraps
 * when width changes).
 *
 * Replaces the inline `previewTexts`, `markdownCache`, `cachedWidth` triple from the previous
 * monolithic `preview-pane.ts`. One Markdown per option, lazy on first request, never
 * re-constructed — count semantics frozen by tests.
 */
export class MarkdownContentCache {
  ...
  constructor(question: QuestionData, theme: Theme, markdownTheme: MarkdownTheme) {
    ...
    for (let i = 0; i < question.options.length; i++) {
      ...
      if (raw !== undefined && raw !== null && raw.length > 0) this.previewTexts.set(i, raw);
    }
  ...
  /**
   * Compute the body lines for a given option at a given inner width. Width changes invalidate the
   * per-Markdown render cache.
   *
   * @param {number} optionIndex - Index of the option to render
   * @param {number} innerWidth - Inner width for markdown reflow
   * @returns {string[]} Rendered body lines
   */
  bodyFor(optionIndex: number, innerWidth: number): string[] {
    ...
    if (text === undefined || text.length === 0) {
      const placeholder = this.theme.fg("dim", uiText("preview.no_preview", NO_PREVIEW_TEXT));
