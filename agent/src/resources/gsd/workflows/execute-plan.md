...
@{{GSD_BUNDLE_DIR}}/references/git-integration.md
</required_reading>
...
2. **MCP tools:** If CLAUDE.md or project instructions reference MCP tools (e.g. jCodeMunch for code navigation), prefer them when available. Otherwise use `grep`/`find` tools.
3. Per task:
...
See `{{GSD_BUNDLE_DIR}}/references/tdd.md` for structure.
</tdd_plan_execution>
...
See {{GSD_BUNDLE_DIR}}/references/checkpoints.md for details.
</step>
...
If `NODE_REPAIR` is `true`: invoke `@{{GSD_BUNDLE_DIR}}/workflows/node-repair.md` with:
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `{{GSD_BUNDLE_DIR}}/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>
...
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `{{GSD_BUNDLE_DIR}}/templates/summary.md`.
