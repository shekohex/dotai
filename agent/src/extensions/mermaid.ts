...
import { AssistantMessageComponent, getMarkdownTheme, keyHint } from "@mariozechner/pi-coding-agent";
		...
		if (closeMatch) {
			...
			if (marker[0] === activeFence.char && marker.length >= activeFence.length) {
				if (activeFence.isMermaid) {
					const block = activeFence.bodyLines.join("").replace(/[\r\n]+$/g, "").trim();
					if (block) {
						blocks.push({
							block,
...
function createMermaidBodyComponent(
	...
	return {
		render: (width) => {
			...
			const selection = selectAsciiVariant(contentWidth, details.variants, details.ascii, details.lineCount);
...
function appendAssistantTextContent(container: Container, text: string, markdownTheme: MarkdownTheme): boolean {
...
function installAssistantMessagePatch(): void {
	...
	prototype.updateContent = function (this: AssistantMessagePatchInstance, message: AssistantMessage): void {
		const hasMermaid = message.content.some(
			(content) => content.type === "text" && content.text.trim() && hasRenderableMermaid(content.text),
		);
		...
		for (let i = 0; i < message.content.length; i++) {
			...
			if (content.type === "text" && content.text.trim()) {
				const rendered = appendAssistantTextContent(this.contentContainer, content.text.trim(), this.markdownTheme);
				if (!rendered) {
			...
			if (this.hideThinkingBlock) {
				this.contentContainer.addChild(
					new Text(interactiveTheme.italic(interactiveTheme.fg("thinkingText", this.hiddenThinkingLabel)), 1, 0),
				);
			} else {
				this.contentContainer.addChild(
		...
		if (message.stopReason === "error") {
			...
			this.contentContainer.addChild(new Spacer(1));
			this.contentContainer.addChild(new Text(interactiveTheme.fg("error", `Error: ${errorMessage}`), 1, 0));
		}
	};
...
function collectRenderableDetails(blocks: string[], notify: (message: string, type: "info" | "warning" | "error") => void): MermaidDetails[] {
