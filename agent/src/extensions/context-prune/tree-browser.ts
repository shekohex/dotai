...
import type { SummaryMessageDetails, ToolCallRecord } from "./types.js";
...
function boxLines(lines: string[], width: number, title: string, _theme: Theme): string[] {
...
export function buildPruneTree(ctx: ExtensionCommandContext, indexer: ToolCallIndexer): TreeNode[] {
  ...
  for (const entry of branch) {
    ...
    if (entry.customType !== CUSTOM_TYPE_SUMMARY) continue;

    const details = normalizeSummaryDetails(entry.details);

    const toolCallRefs = normalizeSummaryToolCallRefs(entry.details);
    const turnIndex = details?.turnIndex ?? "?";
    const timestamp =
      details?.timestamp === undefined ? "" : new Date(details.timestamp).toLocaleString();
    ...
    const summaryText = typeof entry.content === "string" ? entry.content : "";
...
function normalizeSummaryDetails(details: unknown): SummaryMessageDetails | undefined {
  const refs = normalizeSummaryToolCallRefs(details);
  if (!isRecord(details)) {
    return undefined;
  }
  if (typeof details.turnIndex !== "number" || typeof details.timestamp !== "number") {
    return undefined;
  }
  const toolNames = Array.isArray(details.toolNames)
    ? details.toolNames.filter((toolName): toolName is string => typeof toolName === "string")
    : [];
  return {
    toolCallRefs: refs,
    toolNames,
...
function toolCallNode(record: ToolCallRecord, depth: number): TreeNode {
  ...
  const resultPreview = record.resultText.slice(0, 200).replaceAll(/\s+/g, " ");
  return {
...
export class TreeBrowser implements Component {
  ...
  handleInput(data: string): void {
    ...
    if (kb.matches(data, "tui.select.up")) {
      ...
      if (row !== undefined && !row.node.isLeaf) {
  ...
  private openSelectedSummary(): void {
    ...
    if (
      row === undefined ||
      row.node.isLeaf ||
      row.node.detail === undefined ||
      row.node.detail.length === 0
    ) {
      return;
    }
  ...
  private renderWithSummaryOverlay(baseLines: string[], width: number): string[] {
    ...
    if (overlay === null) return baseLines;
