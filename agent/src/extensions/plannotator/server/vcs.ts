...
export const jjRuntime: ReviewJjRuntime = {
  runJj(args: string[], options?: { cwd?: string; timeoutMs?: number }): Promise<GitCommandResult> {
    return runCommand("jj", args, "jj not found", options);
  },
};

const api = createVcsApi([createJjProvider(jjRuntime), createGitProvider(reviewRuntime)]);

export const detectVcs = api.detectVcs.bind(api);
export const getVcsContext = api.getVcsContext.bind(api);
export const detectRemoteDefaultCompareTarget = api.detectRemoteDefaultCompareTarget.bind(api);
export const prepareLocalReviewDiff = api.prepareLocalReviewDiff.bind(api);
export const runVcsDiff = api.runVcsDiff.bind(api);
export const getVcsFileContentsForDiff = api.getVcsFileContentsForDiff.bind(api);
export const canStageFiles = api.canStageFiles.bind(api);
export const stageFile = api.stageFile.bind(api);
export const unstageFile = api.unstageFile.bind(api);
export const resolveVcsCwd = api.resolveVcsCwd.bind(api);
