...
/**
 * Manifest-backed state subcommand router. Keeps gsd-tools.cjs thin while preserving existing
 * command semantics.
 */
function routeStateCommand({ state, args, cwd, raw, parseNamedArgs, error }) {
  ...
  if (subcommand === "json") {
    ...
    } = parseNamedArgs(args, ["phase", "summary", "summary-file", "rationale", "rationale-file"]);
    ...
    const { "keep-recent": keepRecent, "dry-run": dryRun } = parseNamedArgs(
      args,
      ["keep-recent"],
      ["dry-run"],
    );
    ...
    error(
      "state add-roadmap-evolution is SDK-only. Use: gsd-sdk query state.add-roadmap-evolution ...",
    );
