 ...
 *
 * Thin HTTP adapter over the shared annotation store. Mirrors the Bun handler at
 * packages/server/external-annotations.ts but uses node:http IncomingMessage/ServerResponse +
 * res.write() for SSE.
 */
...
/**
 * Create external annotation handler.
 *
 * @param {"plan" | "review"} mode Annotation mode.
 * @returns {{
 *   addAnnotations: (body: unknown) => { ids: string[] } | { error: string };
 *   handle: (req: IncomingMessage, res: ServerResponse, url: URL) => Promise<boolean>;
 * }}
 *   HTTP handler and direct store adapter.
 */
export function createExternalAnnotationHandler(mode: "plan" | "review") {
  ...
  return {
    /**
     * Push annotations directly into the store.
     *
     * @param {unknown} body Annotation payload.
     * @returns {{ ids: string[] } | { error: string }} Created ids or validation error.
     */
    addAnnotations(body: unknown): { ids: string[] } | { error: string } {
    ...
    /**
     * Handle annotation HTTP request.
     *
