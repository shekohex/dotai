...
/**
 * Detect plan-shaped files that the canonical filter would reject. Returns a warning string when
 * offenders exist, else null. Centralised so every read site (phase-plan-index, phases list --type
 * plans, find-phase) emits the same message.
 *
    ...
    if (fs.existsSync(roadmapPath)) {
      try {
        ...
        const phasePattern = new RegExp(
          `#{2,4}\\s*Phase\\s+0*${escapeRegex(normalized)}\\.(\\d+)\\s*:`,
          "gi",
        );
        let pm;
...
function cmdPhaseInsert(cwd, afterPhase, description, raw) {
  ...
  const { decimalPhase, dirName } = withPlanningLock(cwd, () => {
    ...
    const rmPhasePattern = new RegExp(
      `#{2,4}\\s*Phase\\s+0*${escapeRegex(normalizedBase)}\\.(\\d+)\\s*:`,
      "gi",
    );
    let rmMatch;
...
/**
 * Renumber sibling decimal phases after a decimal phase is removed. e.g. removing 06.2 → 06.3
 * becomes 06.2, 06.4 becomes 06.3, etc. Returns { renamedDirs, renamedFiles }.
 */
function renameDecimalPhases(phasesDir, baseInt, removedDecimal) {
  ...
  const toRename = dirs
    .map((dir) => {
      ...
      return m ? { dir, prefix: m[1], oldDecimal: parseInt(m[2], 10), slug: m[3] } : null;
    })
...
/**
 * Renumber all integer phases after removedInt. e.g. removing phase 5 → phase 6 becomes 5, phase 7
 * becomes 6, etc. Returns { renamedDirs, renamedFiles }.
 */
  ...
  withPlanningLock(cwd, () => {
    ...
    if (!isDecimal) {
      ...
      for (let oldNum = MAX_PHASE; oldNum > removedInt; oldNum--) {
        ...
        content = content.replace(
          new RegExp(`(?<![0-9-])${oldPad}-(\\d{2})(?![0-9-])`, "g"),
          `${newPad}-$1`,
        );
