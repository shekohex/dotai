 ...
 *
 * Scans all .planning/ artifact categories for items with open/unresolved state. Returns structured
 * JSON for workflow consumption. Called by: gsd-tools.cjs audit-open Used by:
 * /gsd-complete-milestone pre-close gate
 */
...
/**
 * Scan .planning/debug/ for open sessions. Open = status NOT in ['resolved', 'complete']. Ignores
 * the resolved/ subdirectory.
 */
...
/**
 * Scan .planning/quick/ for incomplete tasks. Incomplete if SUMMARY.md missing or status !==
 * 'complete'.
 */
  ...
  for (const entry of entries) {
    ...
    try {
      const summaryFiles = fs
        .readdirSync(safeTaskDir, { withFileTypes: true })
      ...
      if (summaryFiles.length > 0) {
        ...
        const preferred =
          summaryFiles.find((e) => e.name === `${dirName}-SUMMARY.md`) ||
...
/**
 * Scan .planning/threads/ for open threads. Open if status in ['open', 'in_progress', 'in
 * progress'] (case-insensitive).
 */
...
/**
 * Scan .planning/todos/pending/ for pending todos. Returns array of { filename, priority, area,
 * summary }. Display limited to first 5 + count of remainder.
 */
...
/**
 * Scan .planning/seeds/SEED-*.md for unimplemented seeds. Unimplemented if status in ['dormant',
 * 'active', 'triggered'].
 */
